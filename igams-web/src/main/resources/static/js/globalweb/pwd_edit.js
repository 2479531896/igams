...
function checkpwd(){
	if(!checkPassWord()){
		...
		$.ajax({ 
		    type:'post',  
		    url:"/systemrole/user/pagedataModSavePass",
...
function checkPassWord(){
	var isSuccess = false;
	if($("#input-3").val()==""){
		$("#ymmts .ts_text").text("请输入原密码！");
	}else if($("#input-2").val()==""){
		$("#mmts .ts_text").text("请输入密码！");
	}else if($("#input-3").val()==$("#input-2").val()){
		$("#yzmmts .ts_text").text("新密码与原密码一致，请重新输入！");
	}else if($("#input-4").val()==""){
