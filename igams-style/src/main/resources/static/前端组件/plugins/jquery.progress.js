;(function($) {
	...
	jQuery.ajax = jQuery.wrapAjax = function(options){
		if(!(jQuery.type(options) === "string") && $.founded(options.progressElement) && $(options.progressElement).size() > 0 ){
			...
			return jQuery.__ajax__($.extend(options,{
				//请求发送时的回调函数
			...
			return jQuery.__ajax__(options);
		}
	};
	
	//重写get和post函数
	jQuery.each( [ "get", "post"], function( i, method ) {
		jQuery[ method ] = function( url, data, callback, type , options ) {
			...
			if( $.defined(options) == true && !(jQuery.type(options) === "string") && $.founded(options.progressElement)  == true  && $(options.progressElement).size() > 0 ){
