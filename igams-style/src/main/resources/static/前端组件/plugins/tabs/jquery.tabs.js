		...
		initialize: function(element, options) {
			...
			var templates = {
				container: function() {
					...
					html.push('<a href="#home" aria-controls="home" role="tab" data-bs-toggle="tab">');
...
//						);
				...
				tabdrop: function() {
					...
					html.push('<a href="javascript:void(0);" data-bs-toggle="dropdown" class="dropdown-toggle">');
			...
			if (options.contextmenu) {
		        //obj上禁用右键菜单
				$(element).on('contextmenu', 'li[role=presentation]', function () {
			...
			}).off('click', '.tabdrop li').on('click', '.tabdrop li', function() {
				//折叠菜单点击位置换到最前
				var $prev = $('.tabdrop').prev();
				$('.tabdrop').find('.dropdown-menu').append($prev);
				$('#tab_tab_home').after($(this));
			});
			...
			$(document).off('click', '.change-page-size').on('click', '.change-page-size', function() {
				...
				$('body').find('.gm-global-bg').prepend(title);
			...
			$(window).resize(function() {
				var paneHeight=$(window).height()-$('.gm-nav.top-nav').height()-40;
			...
			$.extend(true, $this, {
				...
				builTab: function(opts) {
					...
					if (!$("#" + id).length) {
						...
						skin=$('.gm-global-bg').attr('data-skin');
						
						//创建新TAB的title
						var title = $(templates.tabtitle(opts));
						//移除title中非文字的部分
						if(title.find('.fa-angle-right,img,.menu-icon-bg')) {
							title.find('.fa-angle-right,img,.menu-icon-bg').remove();
						}
						...
						function addOpenedMark(){
							$('.gm-left-nav-panel').find('a').each(function(){
						...
						$('body').find(".tab-content").prepend(content);
						$('.nav-tabs').show();
	
						
						var number = $('.nav-tabs>li').size();
						//移除所有隐藏的remove图标
						$('.nav-tabs>li>a>i').removeClass('hide');
...
//			            $(element).drop();
				...
				pop: function (id, e) {
			        ...
			        popHtml.css({
			            'top': e[0].offsetHeight +22 + 'px',
			            'left': e[0].offsetLeft + 40 + 'px'
			        });
			        
			        popHtml.appendTo($(element)).fadeIn('slow');
...
//						}
				...
				drop: function() {
					...
					tabs.append(dropdown.find('li')).find('>li').not('.tabdrop').each(function () {
		                if (this.offsetTop > 0 || tabs.width() - $(this).position().left - $(this).width() < 200 && tabs.find('>li').size() > 2) {
