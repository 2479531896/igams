...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input' name='showCount' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			if((_htable.width()-$($t.grid.hDiv).width())>10 && _btable.find('.ddddd').size()==0){
				$(_htable).find('tr').append('<th class="hhhhh" style="width:30px;"></th>');
				$(_btable).find('tr.jqgfirstrow').append('<td role="gridcell" class="ddddd" style="height:0px;width:30px;"></td>');
			}
