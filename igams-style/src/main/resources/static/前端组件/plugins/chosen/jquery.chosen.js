...
;(function($){
  ...
  AbstractChosen = (function() {
    ...
    AbstractChosen.prototype.winnow_results = function() {
      ...
      _ref = this.results_data;
      var $this = this;
      var SearchText_PY = $.getPinyinFirst(escapedSearchText);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        option = _ref[_i];
        ...
        if (this.include_option_in_results(option)) {
          ...
          if (!(option.group && !this.group_search)) {
            ...
            //如果中文已经匹配就不继续使用拼音进行匹配
           	if(!option.search_match && /^[A-Za-z\s]+$/.test(escapedSearchText)){
           		//添加中文拼音首字母筛选功能
                if($.getPinyinFirst){
                	var search_py_regex = null;
                	var chineseFirst_array = $.getPinyinFirst(option.search_text);
                	$.each(SearchText_PY||[],function(i,search_py_item){
                		//匹配拼音不为空
            			if($.founded(search_py_item||"")){
            				search_py_regex = new RegExp(search_py_item, 'i');
            				//循环当前item的拼音组合数组[包含多音组合]
            				$.each(chineseFirst_array||[],function(i,item_py_item){
                        		//匹配拼音不为空
                    			if($.founded(item_py_item||"")){
                    				//先进行大写匹配
      ...
      if (this.options.width != null) {
        return this.options.width;
		  ...
		  if( $ele.parent().find("select.chosen-select").size() > 1){
			  width = $ele.show().outerWidth(true);$ele.hide();				
    ...
    Chosen.prototype.set_up_html = function() {
	    ...
	    Chosen.prototype.activate_field = function() {
	    	...
	    	if($(bootbox).size() > 0){
	    		...
	    		$(bootbox).find('.chosen-container').not(this.container).css('z-index',--z_index);
	    	}else{
	    		this.container.css('z-index','1010')
	    		$('.chosen-container').not(this.container).css('z-index',--z_index);
	    	}
