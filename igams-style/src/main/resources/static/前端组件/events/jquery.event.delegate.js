...
;(function($) {
	 ...
	 * 
	 * 在自动绑定的元素上添加data-bs-toggle="clearfix" data-target="#yxkcmc,#yxkch"
	 *  
	 * */
	$(document).off('click.data-api', '[data-bs-toggle*="clearfix"]').on('click.data-api', '[data-bs-toggle*="clearfix"]', function (event) {
	...
	//只能输入参数自动绑定脚本
	.off('keyup.data-api', '[data-bs-toggle*="float"]').on('keyup.data-api', '[data-bs-toggle*="float"]', function (e) {
	...
	}).on('blur.data-api', '[data-bs-toggle*="float"]', function (e) {
		if($.trim(this.value).length>0){
			var firstIndex = this.value.indexOf('.');
			//以.开始自动在前方添加0
