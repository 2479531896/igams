...
(function($) {
    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        target.handleNode1 = function(parentNode, lv, row_id, p_id, tbody,index,_row_id) {
            ...
            $(this).find("tbody").find("tr").find(".treetable-expander").off("click")
            target.find("tbody").find("tr").find(".treetable-expander").click(function(){
                ...
                if(_isExpanded||_isCollapsed){
                    ...
                    if(_isExpanded){
                        ...
                        var _self = $(this);
                        var _parms = {};
                        // _parms[options.parentCode] = _self.attr('class').split(" ")[0].split("-")[1];
                        _parms[options.parentCode] = _self.parent().parent().attr('id');
                        _parms["sbid"] = _self.parent().next().attr('title');
                        _parms["pid"] = _self.parent().parent().attr('pid');
                        _parms["lv"] = _self.parent().parent().attr('lv');
                        target.loadChilds(_parms);
        ...
        target.renderRow = function(item,isP,lv,row_id,p_id){
            // 标记已显示
            item.isShow = true;
            var tr = $('<tr id="'+row_id+'" pid="'+p_id+'" lv="'+lv+'"></tr>');
            var _icon = options.expanderCollapsedClass;
            var isFind = false;
            $.each(preExpendData,function(_i,_item){
            	if(_item.id ==item[options.id]){
            		tr.css("display","table");
	                _icon = options.expanderExpandedClass;
	                isFind =true;
            	}
            	if(_item.id ==item[options.fid]){
            		tr.css("display","table");
	                _icon = options.expanderExpandedClass;
	                isFind =true;
            	}
            ...
            $.each(options.columns, function(index, column) {
                ...
                if(column.field=='selectItem'){
                    ...
                    var td = $('<td title="'+item[column.field]+'" name="'+column.field+'" style="'+((column.width)?('width:'+column.width):'')+((column.invisible)?(';display:none;'):'')+'"></td>');
                    // 增加formatter渲染
                        if (column.formatter) {
        ...
        target.loadChilds=function (parms) {
            var _ls = allData["_n_"+parms["sbid"]];

            var fsbid=parms["fsbid"]
            var lv=parms["lv"]
            var lvArr=map.get(lv)
            if(lvArr==null){
                lvArr=new Array()
            }
            var rowid=lvArr.find(item=>item==fsbid)
            if(rowid==null){
                lvArr.push(fsbid)
            }
            map.set(lv,lvArr)
            if(_ls){
                    $.each(_ls, function(i, item) {
                        var _row_id = fsbid+"_"+i
                        $("#"+_row_id).remove();
                        var tbody = target.find("tbody");
                        target.handleNode1(item, parseInt(parseInt(lv) + 1), _row_id,fsbid, tbody,i,fsbid+"_"+(i-1))
                    });
        ...
        target.load = function(parms){
            ...
            if(allData != null){
            	preExpendData=[];
            	var alld = target.find("tbody").find("tr").find(".glyphicon-chevron-down");
                $.each(alld,function(_i,_item){
                	var _data = {id:$(_item).parent().parent().find('[name="'+options.id+'"]').attr("title")};
                	var _tds = $(_item).parent().parent().find("td");
	                _tds.each(function(_i,_subitem){ 
	                    if(_i!=0){
	                        _data[$(_subitem).attr("name")]=$(_subitem).attr("title");
	                    }
	                }); 
	                preExpendData.push(_data); 
                }); 
            }
            allData = {};
            // 加载数据前先清空
            ...
            $.each(options.columns, function(i, item) {
                ...
                if(i==0&&item.field=='selectItem'){
                    ...
                    th = $('<th style="'+((item.width)?('width:'+item.width):'')+((item.invisible)?(';display:none;'):'')+'"></th>');
                }
                th.text(item.title);
                thr.append(th);
            });
                ...
                map.forEach((val,key) => {
                    ...
                    if(arr!=null){
                        for(var i=0;i<arr.length;i++){
                            var id=arr[i]
                            $("#"+id).find(".treetable-expander").click()
                            $("#"+id).find(".treetable-expander").click()
                        }
                    }
                })
