	...
	a.extend({
		...
		alert: function(g, b, d) {
			...
			if (g && g.indexOf("timeSettingInfo") > 10) {
				...
				a.message(a.i18n.bootbox.messages["open_tip"], g, b, d)
			} else {
				...
				bootbox.alert(a.i18n.bootbox.titles["alert"], '<div class="alert alert-modal"><p>' + g + "</p></div>", function(h) {
					if (a.isFunction(b)) {
		...
		error: function(g, b, d) {
			...
			if (g && g.indexOf("timeSettingInfo") > 10) {
				...
				bootbox.alert(a.i18n.bootbox.titles["error"], '<div class="error error-modal"><p>' + g + "</p></div>", function(h) {
					if (a.isFunction(b)) {
						return b.call(this, h)
					} else {
						return true
		...
		success: function(g, b, d) {
			...
			if (g && g.indexOf("timeSettingInfo") > 10) {
				...
				bootbox.alert(a.i18n.bootbox.titles["success"], '<div class="success success-modal"><p>' + g + "</p></div>", function(h) {
					if (a.isFunction(b)) {
						return b.call(this, h)
					} else {
						return true
		...
		confirm: function(g, b, d) {
			...
			if (g && g.indexOf("timeSettingInfo") > 10) {
				...
				bootbox.confirm(a.i18n.bootbox.titles["confirm"], '<div class="alert confirm-modal"><p>' + g + "</p></div>", function(h) {
					if (a.isFunction(b)) {
						return b.call(this, h)
					} else {
						return true
		...
		prompt: function(b, c) {
			...
			bootbox.prompt(a.i18n.bootbox.titles["prompt"], function(d) {
				if (a.isFunction(b)) {
		...
		dialog: function(b) {
			...
			var d = {
				...
				onShow: null,
				onShown: function() {
					a('[data-bs-toggle*="validation"]')
						.trigger("validation");
					a('[data-bs-toggle*="fixed"]')
						.trigger("fixed");
...
//							})
					...
					if (a.fn.fixed && b.fixedTarget && a(b.fixedTarget)
						.length == 1) {
						a(b.fixedTarget)
							.fixed({
								scrollElement: a("#" + b.modalName),
								container: a("#" + b.modalName)
									.find(".modal-dialog"),
								z_index: a("#" + b.modalName)
									.css("z-index")
							})
					}
				}
			...
			const resizer = $("#" + b.modalName+" .ui-draggable");
			var moveDia = function(flg){
				moveflg = flg;
			...
			var onMouseDown = function(e){
		        ...
		        startX = e.clientX;
		        startY = e.clientY;
		        startWidth = resizer.width();
		        startTop = resizer.offset().top;
		        startLeft = resizer.offset().left;
		        //startHeight = resizer.height();
		        startHeight = $("#" + b.modalName+" .ui-draggable .bootbox-body").height();
			...
			var onMouseMove = function(e){
		        ...
		        //console.log("11X:" + startX + " Y:" + startY + " deltaX:" + deltaX + " deltaY:" + deltaY + " X1:" + e.clientX + " Y1:" +  e.clientY + " flg:"+moveflg)
		        if(moveflg ==1){
					resizer.css("left",e.clientX- 17);
		        	resizer.css("width",(startWidth - deltaX));
				}else if(moveflg ==2){
		        	resizer.css("width",(startWidth + deltaX));
				}else if(moveflg ==3){
					resizer.css("left",startLeft- 17);
					resizer.css("top",startTop);
					$("div .ui-draggable .bootbox-body").css("max-height",(startHeight + deltaY));
		        	//resizer.css("height",(startHeight + deltaY));
				}else if(moveflg ==11){
					resizer.css("left",e.clientX- 17);
					resizer.css("top",startTop);
		        	resizer.css("width",(startWidth - deltaX));
					...
					resizer.css("top",startTop);
		        	resizer.css("width",(startWidth + deltaX));
		        	$("#" + b.modalName+" .ui-draggable .bootbox-body").css("max-height",(startHeight + deltaY));
		        	//resizer.css("height",(startHeight + deltaY));
				}
		    };
		    var onMouseUp = function(e){
				const deltaX = e.clientX - startX;
		        const deltaY = e.clientY - startY;
				//console.log("X:" + startX + " Y:" + startY + " deltaX:" + deltaX + " deltaY:" + deltaY + " X1:" + e.clientX + " Y1:" +  e.clientY + " flg:"+moveflg)
		    	isResizing = false;
			...
			$("div .ui-draggable .modal-content").mousemove(function(e) {
				if(isResizing)
					return;
				var width = $("div .ui-draggable .modal-content").width();
				var height = $("div .ui-draggable .modal-content").height();
				var mouseX = e.clientX - $("div .ui-draggable .modal-content").offset().left; // 鼠标位置相对于div的左上角
				var mouseY = e.clientY - $("div .ui-draggable .modal-content").offset().top;
				//console.log("width:" + width + " height:" + height + " mouseX:" + mouseX + " mouseY:" + mouseY + " left:" + $("div .ui-draggable .modal-content").offset().left + " top:" + $("div .ui-draggable .modal-content").offset().top)
				//左边
				if (mouseX >= 0 && mouseX <= 10 && mouseY > 10 && mouseY < height - 10) {
					//console.log("left")
					$("div .ui-draggable .modal-content").css("cursor","w-resize");
					$("div .ui-draggable .modal-content .ui-draggable-handle").css("cursor","move");
					moveDia(1);
				}else if(mouseX >= width - 10  && mouseY > 10 && mouseY < height - 10) {
					//console.log("right")
					//右
					$("div .ui-draggable .modal-content").css("cursor","e-resize");
					$("div .ui-draggable .modal-content .ui-draggable-handle").css("cursor","move");
					moveDia(2);
				}else if(mouseX > 10 && mouseX <= width - 10 && mouseY >= height - 10) {
					//console.log("bottom")
					//下
					$("div .ui-draggable .modal-content").css("cursor","s-resize");
					$("div .ui-draggable .modal-content .ui-draggable-handle").css("cursor","s-resize");
					moveDia(3);
				}
				else if(mouseX >= 0 && mouseX <= 10 && mouseY >= height - 10) {
					//console.log("left_bottom")
					//左下
					$("div .ui-draggable .modal-content").css("cursor","sw-resize");
					$("div .ui-draggable .modal-content .ui-draggable-handle").css("cursor","sw-resize");
					moveDia(11);
				}
				else if(mouseX >= width - 10 && mouseY >= height - 10) {
					//console.log("right_bottom")
					//右下
					$("div .ui-draggable .modal-content").css("cursor","se-resize");
					$("div .ui-draggable .modal-content .ui-draggable-handle").css("cursor","se-resize");
					moveDia(12);
				}else{
					//console.log("default")
					$("div .ui-draggable .modal-content").css("cursor","auto");
					$("div .ui-draggable .modal-content .ui-draggable-handle").css("cursor","move");
					//resizer.off('mousedown', onMouseDown);
	...
	a.exportDialog = function(b, d, l, m, g, p) {
		...
		if (m && m.length > 0) {
			...
			for (var h = 0; h < m.length; h++) {
				var f = m[h];
				if (a.trim(f.label)
					.length > 0 && a.trim(f.index)
					.length > 0 && f.hidden != true) {
...
function delphoneredis(e) {
	jQuery.ajaxSetup({
		async: !1
	});
	jQuery.post("/common/file/pagedataDelPhoneUpload", {
		ywlx: $(e + " #ywlx")
			.val(),
		ywid: $(e + " #phoneywid")
			.val(),
