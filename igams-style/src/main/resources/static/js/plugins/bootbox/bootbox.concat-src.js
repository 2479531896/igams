...
(function (root, factory) {
  ...
  exports.loadBody = function (dialog,options,def){
	  if(dialog && options && $.trim(options.href).length>0){
		  ...
		  if(modal_body.length > 0){
			  ...
			  window.api = {
				  data 		: options.data||{},
				  opener 	: $("div.bootbox").length>0?$("div.bootbox").last():document.body,
				  document 	: dialog.find(".bootbox-body")
			  };
			  var paramMap = $.extend(true,{},options.data||{});
			  delete paramMap.colModel;
  ...
  exports.dialog = function(options) {
    ...
    options.opener = $("div.bootbox").length>0?$("div.bootbox").last():document.body;
    ...
    function _getDimension(){
    	...
    	// 导航栏高度
    	var _navbarHeight = $.fn.actual ? $("#tabs1").actual("outerHeight") : $("#tabs1").outerHeight(true);
        // jQuer 3.*不支持 _navbarHeight可能为undefined的情况 ，故增加该行代码
        _navbarHeight = _navbarHeight ? _navbarHeight : 0;
    	// 内容区域高度
    ...
    dialog.on("loaded.bs.modal", function() {
	   	//页面加载完成后的回调函数
	   	options.loaded.call(this,dialog);
    ...
    dialog.on("hidden.bs.modal", function(e) {
	     ...
	     //判断是否有固定区域逻辑
	   	 if ($.fn.fixed && options.fixedTarget && $(options.fixedTarget).length==1) {
    ...
    dialog.on("resize.bs.modal", function(event) {
    ...
    dialog.on("click", ".bootbox-close-button", function(e) {
        var timers=$("#"+options.modalName).attr("dsq")
        if(timers){
            ...
            for(var i=0;i<arr.length;i++){
                if(arr[i]!=""){
                   ...
                   delphoneredis("#"+options.modalName);
                }
	...
	$.when(dialog.data("def")).done(function(){
        ...
        $(window).resize(function(event){
			...
			$.when($.Deferred(function(def){
				...
				//触发尺寸变化
				dialog.trigger("resized.bs.modal");
				//延时改变状态，方式重置事件太快导致卡顿
			  	window.setTimeout(function(){
			  		// 改变deferred对象的执行状态为：已完成
				    def.resolve(); 
...
;(function($) {
	...
	$.extend({
		...
		dialog:function(options){
			var defaults = {
				...
				loaded		: function(){
					/*触发验证事件：实现自动绑定验证*/
					$('[data-bs-toggle*="validation"]').trigger("validation");
					$('[data-bs-toggle*="fixed"]').trigger("fixed");
					if($.fn.tooltip){
	...
	$.exportDialog = function(href,dcclbh,requestMap,colModel,gridId,callback){
		...
		//初始化导出配置
		var initURL = _path + '/matridx/drdc/export_exportInitDcpz.html';
		...
		if($.founded(gridId) && $(gridId).length > 0 ){
			//排序字段
		...
		$.dialog({
			"title"		: '自定义导出',
			"href"		: _path + '/matridx/drdc/export_exportConfig.html',
			"data"		: {"dcclbh":dcclbh},
			"width"		: "900px",
			"modalName"	: "ExportModal",
			/*"afterRender":function(){
				if($(this.document).find("#exportForm").length == 0){
					this.button({
	                    id		:'bcdc',
	                    disabled: true
	                });
...
function delphoneredis(formid){
     jQuery.ajaxSetup({async:false});
                        var url= "/common/file/pagedataDelPhoneUpload";
                        jQuery.post(url,{"ywlx":$(formid+" #ywlx").val(),"ywid":$(formid+" #phoneywid").val(),"access_token":$("#ac_tk").val()},function(responseText){
                            setTimeout(function(){
