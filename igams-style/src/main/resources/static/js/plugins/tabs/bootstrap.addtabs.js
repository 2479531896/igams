...
(function ($) {
    ...
    var settings = {
         ...
         * @type {String}
         */
        contentStyle: 'content divscroll',
        /**
         * ajax 的参数
         * @type {Object}
         */
    ...
    _click = function (obj) {
        ...
        $.addtabs.add({
            ...
            'tohref':a_obj.tohref ? a_obj.tohref : obj.attr('tohref'),
        });
    ...
    _pop = function (id, e, mouse) {
        ...
        var popHtml = $('<ul>', {
            ...
            'aria-tohref': e.attr('aria-tohref')
        }).append(refresh)
            .append(remove)
        ...
        $('ul.rightMenu a[data-right=refresh]').on('click', function () {
            ...
            var tohref = $(this).parent('ul').attr('aria-tohref');
            $.addtabs.add({
                'id': id,
                'url': url,
                'refresh': true,
    ...
    $.addtabs.add = function (opts) {
        ...
        tab_li.children('li[role = "presentation"]').find('a').removeClass('active');
        ...
        if (tab_li.find('#tab_' + id).length < 1) {
            ...
            var title = $('<li>', {
                ...
                'aria-tohref': opts.tohref==null ? true : false,
                'class':'nav-item '
            }).append(
                $('<a>', {
                    'href': '#' + id,
        ...
        var flag =false;
        if(((opts.url+"").indexOf("https://")==-0)||(opts.url+"").indexOf("http://")==0){
            flag=true
        }
        if(flag){
            vueid=opts.id;
            ...
            opts.ajax=false
            var reg= new RegExp("(^|&)tohref=([^&]*)(&|$)", "i");
            var r = window.location.search.substr(1).match(reg);
            if(r!=null&&opts.tohref!="false"){
                var tohref=unescape(r[2])
                var por=opts.url.split("://");
                var ipdz=por[1].split("/");

                opts.url=por[0]+"://"+ipdz[0]+decodeURIComponent(tohref)
            }
        }else{
...
//        tab_li.find('#tab_' + id).addClass('active');
        tab_li.find('#tab_' + id+'>a').addClass('active');
        console.log(321)
        tab_content.find('#' + id).addClass('active');
        tab_content.find('#' + id).find('#tabCover').remove();
        if(!opts.refresh){
            $('#tab_' + id+" .tab-pane").css("height",$(".gm-left-nav").height()-$(".nav-tabs").height()+40);
            $('#tab_' + id+" .iframeClass").css("height",$(".gm-left-nav").height()-$(".nav-tabs").height()+30);
        }
    ...
    $.addtabs.close = function (opts) {
        ...
        if ($("#tab_" + opts.id+'>a').hasClass('active')) {
            if ($('#tab_' + opts.id).parents('li.tabdrop').length > 0 && !$('#tab_' + opts.id).parents('li.tabdrop').hasClass('hide')) {
                ...
                $("#tab_" + opts.id).prev('li').find('a').tab('show');
            }
            $("#" + opts.id).prev().addClass('active');
            $("#" + opts.id).prev().removeClass('show');
        }
        //关闭TAB
        $("#tab_" + opts.id).remove();
        $("#" + opts.id).remove();
    ...
    $.addtabs.drop = function () {
        ...
        $('body').find('#tabs1 .nav-tabs').each(function () {
        consle.log(88881)
            var element = $(this);
            //检测是否已增加
