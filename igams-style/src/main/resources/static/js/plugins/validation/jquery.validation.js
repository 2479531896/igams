...
//公共的form提交函数
function submitForm(formID,callback,btnDomSelector){
	preventResubmitForm(btnDomSelector,null,formID);
...
;(function($){
		...
		initialize 	: function(form, options) {
	    	...
	    	var settings =  $.extend({}, options, {
	    		...
	    		success : function(label, element) {
	    			...
	    			$(label).remove();
	    			//适配不同页面环境的提示样式元素
	    			var errorElement = getTargetElement(element);
	    			//移除样式
	    		...
	    		errorPlacement : function(label, element) {
	    			...
	    			if("wrap" == options.tipMethod){
	    				...
	    				var options2  	= 	{
		    				...
		    				trigger			:   "hover focus",
		    				container		: 	(jqgrid.length > 0 ? jqgrid : (bootbox.length > 0 ? bootbox : "body"))
	    				};
	    		...
	    		submitHandler : function(form) {
				   ...
				   if(status){
					   ...
					   var localbar = $(progressbar).length == 1 && !$.founded(options.progressURL);
					   var remotebar = $(progressbar).length == 1 && $.founded(options.progressURL);
	...
	$.fn.validateForm.defaults = {
		...
		onError	: function(xhr, statusText, errMsg,setting){
			if(typeof afterSuccessFunc == "function"){
				afterSuccessFunc.call(this,xhr,statusText);
			}
		},
   		//请求完成的回调函数
	 ...
	 * 
	 * 在验证的form上添加 data-bs-toggle="validation"
	 * */
	$(document).on('validation.data-api','[data-bs-toggle*="validation"]', function (event) {
...
*/				var isValid = true;
		...
		clearValid : function(){
			if ( $(this[0]).is("form")) {
    			...
    			//适配不同页面环境的提示样式元素
				$(getTargetElement(this)).removeClass("has-error has-warning has-success");
				if($.fn.tooltips){
					$(this).getRealElement().tooltips('destroy');
				}
			}
