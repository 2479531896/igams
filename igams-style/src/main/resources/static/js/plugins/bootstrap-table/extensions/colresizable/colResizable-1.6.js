...
(function($){ 	
	...
	var init = function( tb, options){
		...
		if(options.isFirst && t.width() > 767){
			if(window.parent==this.window&&(t.width()>=$(".fixed-table-body").width())){
				t.width(t.width()- 68);
			}
		}
			//t.width(t.width()- 68);
		...
		if(t.opt.disable) return destroy(t,options);				//the user is asking to destroy a previously colResized table
		var	id = t.id = t.attr(ID) || SIGNATURE+count++;	//its id is obtained, if null new one is generated		
		t.p = t.opt.postbackSafe; 							//short-cut to detect postback safe 		
		if(!t.is("table") || tables[options.pid+"-"+id] && !t.opt.partialRefresh) return; 		//if the object is not a table or if it was already processed then it is ignored.
		...
		tables[options.pid+"-"+id] = t; 	//the table object is stored using its id as key	
	 ...
	 */
	var destroy = function(t,options){
		var id=t.attr(ID), t=tables[options.pid+"-"+id];		//its table object is found
		if(!t||!t.is("table")) return;			//if none, then it wasn't processed	 
		t.removeClass(SIGNATURE+" "+FLEX).gc.remove();	//class and grips are removed
		delete tables[options.pid+"-"+id];						//clean up data
	};
	 ...
	 * @param {jQuery ref} t - table object
	 */
	var createGrips = function(t,options){
		...
		th.each(function(i){						//iterate through the table column headers			
			...
			g.data(SIGNATURE, {i:i, t:t.attr(ID), last: i == t.ln-1,pid:options.pid});	 //grip index and its table name are stored in the HTML 												
		});
	...
	*/
	...
	var onGripMouseDown = function(e){
		...
		var t = tables[o.pid+"-"+o.t],  g = t.g[o.i];			//shortcuts for the table and grip objects
