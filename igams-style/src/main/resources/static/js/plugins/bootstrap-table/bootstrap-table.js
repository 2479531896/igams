  ...
  (function ($) {
    ...
    var constants = {
      ...
      4: {
        ...
        html: {
          ...
          pageDropdown: ['<ul class="dropdown-menu">', '</ul>'],
          pageDropdownItem: '<li><a class="dropdown-item %s" href="#">%s</a><li>',
          dropdownCaret: '',//<span class="caret"></span>
          pagination: ['<ul class="pagination%s">', '</ul>'],
          paginationItem: '<li class="page-item%s"><a class="page-link" href="#">%s</a></li>',
          icon: '<i class="%s %s"></i>'
    ...
    var BootstrapTable = function () {
      ...
      _createClass(BootstrapTable, [{
          ...
          if (o.showColumns) {
            ...
            this.columns.forEach(function (column, i) {
              ...
              if (column.switchable) {
                html.push(Utils.sprintf(_this4.constants.html.toobarDropdowItem, Utils.sprintf('<input type="checkbox" data-field="%s" name="chk_bt_%s" value="%s"%s> <span>%s</span>', column.field,column.field, i, checked, column.title)));
                switchableCount++;
              }
            });
        ...
        value: function initServer(silent, query, url) {
          ...
          var _this9 = this;
          var formStr=this. options.toolbar
          var formid="";
          if(formStr!=null&&formStr!=undefined){
            formid=formStr.split(" ")[0]
          }
          if(formid!=""){
            $(formid+" #btn_query").prop("disabled",true)
            console.log(123)
          ...
          var request = $.extend({}, Utils.calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            success: function success(_res) {
              ...
              if(formid!=""){
                  $(formid+" #btn_query").prop("disabled",false)
    ...
    $.fn.bootstrapTable = function (option) {
      ...
      this.each(function (i, el) {
        ...
        //如果屏幕宽度小于767px,自动设置详细显示
        if(!options.isForceTable){
	        if($(this).width() <= 767){
	        	var showColumnsCount = 0;
	        	for(var x=0;x<options.columns.length;x++){
	        		if (!options.columns[x].visible) {
	                    continue;
	                }
	        		showColumnsCount ++ ;
	        	}
	        	if(showColumnsCount > 6 || showColumnsCount == 0)
