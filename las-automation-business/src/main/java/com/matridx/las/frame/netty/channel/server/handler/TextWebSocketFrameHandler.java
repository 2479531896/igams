...
public class TextWebSocketFrameHandler extends MatridxProtocolHandler<TextWebSocketFrame>{

	public static ChannelGroup socketChannels = new DefaultChannelGroup(GlobalEventExecutor.INSTANCE);

    Logger log = LoggerFactory.getLogger(TextWebSocketFrameHandler.class);
    ...
    */
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {  // (2)
        Channel incoming = ctx.channel();
        for (Channel channel : socketChannels) {
            channel.writeAndFlush(new TextWebSocketFrame("[SERVER] - " + incoming.remoteAddress() + " 加入2"));
        }
        socketChannels.add(ctx.channel());
        System.out.println("Client:"+incoming.remoteAddress() +"加入2");
    }
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception { // (5)
        Channel incoming = ctx.channel();
        System.out.println("Client:"+incoming.remoteAddress()+"在线2");
    }

    @Override
    public void channelInactive(ChannelHandlerContext ctx) throws Exception { // (6)
        Channel incoming = ctx.channel();
        System.out.println("Client:"+incoming.remoteAddress()+"掉线2");
    }
