package com.matridx.igams.production.dao.entities;

import org.apache.ibatis.type.Alias;

@Alias(value = "HY_DZ_BorrowOutModel")
public class HY_DZ_BorrowOutModel {


    private String ID;
    private String cCreateType;
    private String cCODE;
    private String cType;
    private String bObjectCode;
    private String cpersoncode;
    private String cContactperson;
    private String cContactWay;
    private String cdepcode;
    private String ddate;
    private String cmemo;
    private String cfreight;
    private String cfreightType;
    private String cfreightCompany;
    private String cfreightCost;
    private String cAboutVoucher;
    private String cCodeAboutVoucher;
    private String cMaker;
    private String dmDate;
    private String cHandler;
    private String dVeriDate;
    private String CloseUser;
    private String dCloseDate;
    private String IntoUser;
    private String dIntoDate;
    private String iStatus;
    private String VoucherType;
    private String iverifystate;
    private String iswfcontrolled;
    private String ireturncount;
    private String cdefine1;
    private String cdefine2;
    private String cdefine3;
    private String cdefine4;
    private String cdefine5;
    private String cdefine6;
    private String cdefine7;
    private String cdefine8;
    private String cdefine9;
    private String cdefine10;
    private String cdefine11;
    private String cdefine12;
    private String cdefine13;
    private String cdefine14;
    private String cdefine15;
    private String cdefine16;
    private String ufts;
    private String MycdefineT1;
    private String MycdefineT2;
    private String MycdefineT3;
    private String MycdefineT4;
    private String MycdefineT5;
    private String MycdefineT6;
    private String MycdefineT7;
    private String MycdefineT8;
    private String MycdefineT9;
    private String MycdefineT10;
    private String DownstreamCode;
    private String UpStreamCode;
    private String iPrintCount;
    private String iVTID;
    private String csysbarcode;
    private String cborrowouttype;
    private String type_id;
    private String rel_obj_type;
    private String rel_obj_id;
    private String rel_obj_stageid;


    public String getID() {
        return ID;
    }

    public void setID(String ID) {
        this.ID = ID;
    }

    public String getcCreateType() {
        return cCreateType;
    }

    public void setcCreateType(String cCreateType) {
        this.cCreateType = cCreateType;
    }

    public String getcCODE() {
        return cCODE;
    }

    public void setcCODE(String cCODE) {
        this.cCODE = cCODE;
    }

    public String getcType() {
        return cType;
    }

    public void setcType(String cType) {
        this.cType = cType;
    }

    public String getbObjectCode() {
        return bObjectCode;
    }

    public void setbObjectCode(String bObjectCode) {
        this.bObjectCode = bObjectCode;
    }

    public String getCpersoncode() {
        return cpersoncode;
    }

    public void setCpersoncode(String cpersoncode) {
        this.cpersoncode = cpersoncode;
    }

    public String getcContactperson() {
        return cContactperson;
    }

    public void setcContactperson(String cContactperson) {
        this.cContactperson = cContactperson;
    }

    public String getcContactWay() {
        return cContactWay;
    }

    public void setcContactWay(String cContactWay) {
        this.cContactWay = cContactWay;
    }

    public String getCdepcode() {
        return cdepcode;
    }

    public void setCdepcode(String cdepcode) {
        this.cdepcode = cdepcode;
    }

    public String getDdate() {
        return ddate;
    }

    public void setDdate(String ddate) {
        this.ddate = ddate;
    }

    public String getCmemo() {
        return cmemo;
    }

    public void setCmemo(String cmemo) {
        this.cmemo = cmemo;
    }

    public String getCfreight() {
        return cfreight;
    }

    public void setCfreight(String cfreight) {
        this.cfreight = cfreight;
    }

    public String getCfreightType() {
        return cfreightType;
    }

    public void setCfreightType(String cfreightType) {
        this.cfreightType = cfreightType;
    }

    public String getCfreightCompany() {
        return cfreightCompany;
    }

    public void setCfreightCompany(String cfreightCompany) {
        this.cfreightCompany = cfreightCompany;
    }

    public String getCfreightCost() {
        return cfreightCost;
    }

    public void setCfreightCost(String cfreightCost) {
        this.cfreightCost = cfreightCost;
    }

    public String getcAboutVoucher() {
        return cAboutVoucher;
    }

    public void setcAboutVoucher(String cAboutVoucher) {
        this.cAboutVoucher = cAboutVoucher;
    }

    public String getcCodeAboutVoucher() {
        return cCodeAboutVoucher;
    }

    public void setcCodeAboutVoucher(String cCodeAboutVoucher) {
        this.cCodeAboutVoucher = cCodeAboutVoucher;
    }

    public String getcMaker() {
        return cMaker;
    }

    public void setcMaker(String cMaker) {
        this.cMaker = cMaker;
    }

    public String getDmDate() {
        return dmDate;
    }

    public void setDmDate(String dmDate) {
        this.dmDate = dmDate;
    }

    public String getcHandler() {
        return cHandler;
    }

    public void setcHandler(String cHandler) {
        this.cHandler = cHandler;
    }

    public String getdVeriDate() {
        return dVeriDate;
    }

    public void setdVeriDate(String dVeriDate) {
        this.dVeriDate = dVeriDate;
    }

    public String getCloseUser() {
        return CloseUser;
    }

    public void setCloseUser(String closeUser) {
        CloseUser = closeUser;
    }

    public String getdCloseDate() {
        return dCloseDate;
    }

    public void setdCloseDate(String dCloseDate) {
        this.dCloseDate = dCloseDate;
    }

    public String getIntoUser() {
        return IntoUser;
    }

    public void setIntoUser(String intoUser) {
        IntoUser = intoUser;
    }

    public String getdIntoDate() {
        return dIntoDate;
    }

    public void setdIntoDate(String dIntoDate) {
        this.dIntoDate = dIntoDate;
    }

    public String getiStatus() {
        return iStatus;
    }

    public void setiStatus(String iStatus) {
        this.iStatus = iStatus;
    }

    public String getVoucherType() {
        return VoucherType;
    }

    public void setVoucherType(String voucherType) {
        VoucherType = voucherType;
    }

    public String getIverifystate() {
        return iverifystate;
    }

    public void setIverifystate(String iverifystate) {
        this.iverifystate = iverifystate;
    }

    public String getIswfcontrolled() {
        return iswfcontrolled;
    }

    public void setIswfcontrolled(String iswfcontrolled) {
        this.iswfcontrolled = iswfcontrolled;
    }

    public String getIreturncount() {
        return ireturncount;
    }

    public void setIreturncount(String ireturncount) {
        this.ireturncount = ireturncount;
    }

    public String getCdefine1() {
        return cdefine1;
    }

    public void setCdefine1(String cdefine1) {
        this.cdefine1 = cdefine1;
    }

    public String getCdefine2() {
        return cdefine2;
    }

    public void setCdefine2(String cdefine2) {
        this.cdefine2 = cdefine2;
    }

    public String getCdefine3() {
        return cdefine3;
    }

    public void setCdefine3(String cdefine3) {
        this.cdefine3 = cdefine3;
    }

    public String getCdefine4() {
        return cdefine4;
    }

    public void setCdefine4(String cdefine4) {
        this.cdefine4 = cdefine4;
    }

    public String getCdefine5() {
        return cdefine5;
    }

    public void setCdefine5(String cdefine5) {
        this.cdefine5 = cdefine5;
    }

    public String getCdefine6() {
        return cdefine6;
    }

    public void setCdefine6(String cdefine6) {
        this.cdefine6 = cdefine6;
    }

    public String getCdefine7() {
        return cdefine7;
    }

    public void setCdefine7(String cdefine7) {
        this.cdefine7 = cdefine7;
    }

    public String getCdefine8() {
        return cdefine8;
    }

    public void setCdefine8(String cdefine8) {
        this.cdefine8 = cdefine8;
    }

    public String getCdefine9() {
        return cdefine9;
    }

    public void setCdefine9(String cdefine9) {
        this.cdefine9 = cdefine9;
    }

    public String getCdefine10() {
        return cdefine10;
    }

    public void setCdefine10(String cdefine10) {
        this.cdefine10 = cdefine10;
    }

    public String getCdefine11() {
        return cdefine11;
    }

    public void setCdefine11(String cdefine11) {
        this.cdefine11 = cdefine11;
    }

    public String getCdefine12() {
        return cdefine12;
    }

    public void setCdefine12(String cdefine12) {
        this.cdefine12 = cdefine12;
    }

    public String getCdefine13() {
        return cdefine13;
    }

    public void setCdefine13(String cdefine13) {
        this.cdefine13 = cdefine13;
    }

    public String getCdefine14() {
        return cdefine14;
    }

    public void setCdefine14(String cdefine14) {
        this.cdefine14 = cdefine14;
    }

    public String getCdefine15() {
        return cdefine15;
    }

    public void setCdefine15(String cdefine15) {
        this.cdefine15 = cdefine15;
    }

    public String getCdefine16() {
        return cdefine16;
    }

    public void setCdefine16(String cdefine16) {
        this.cdefine16 = cdefine16;
    }

    public String getUfts() {
        return ufts;
    }

    public void setUfts(String ufts) {
        this.ufts = ufts;
    }

    public String getMycdefineT1() {
        return MycdefineT1;
    }

    public void setMycdefineT1(String mycdefineT1) {
        MycdefineT1 = mycdefineT1;
    }

    public String getMycdefineT2() {
        return MycdefineT2;
    }

    public void setMycdefineT2(String mycdefineT2) {
        MycdefineT2 = mycdefineT2;
    }

    public String getMycdefineT3() {
        return MycdefineT3;
    }

    public void setMycdefineT3(String mycdefineT3) {
        MycdefineT3 = mycdefineT3;
    }

    public String getMycdefineT4() {
        return MycdefineT4;
    }

    public void setMycdefineT4(String mycdefineT4) {
        MycdefineT4 = mycdefineT4;
    }

    public String getMycdefineT5() {
        return MycdefineT5;
    }

    public void setMycdefineT5(String mycdefineT5) {
        MycdefineT5 = mycdefineT5;
    }

    public String getMycdefineT6() {
        return MycdefineT6;
    }

    public void setMycdefineT6(String mycdefineT6) {
        MycdefineT6 = mycdefineT6;
    }

    public String getMycdefineT7() {
        return MycdefineT7;
    }

    public void setMycdefineT7(String mycdefineT7) {
        MycdefineT7 = mycdefineT7;
    }

    public String getMycdefineT8() {
        return MycdefineT8;
    }

    public void setMycdefineT8(String mycdefineT8) {
        MycdefineT8 = mycdefineT8;
    }

    public String getMycdefineT9() {
        return MycdefineT9;
    }

    public void setMycdefineT9(String mycdefineT9) {
        MycdefineT9 = mycdefineT9;
    }

    public String getMycdefineT10() {
        return MycdefineT10;
    }

    public void setMycdefineT10(String mycdefineT10) {
        MycdefineT10 = mycdefineT10;
    }

    public String getDownstreamCode() {
        return DownstreamCode;
    }

    public void setDownstreamCode(String downstreamCode) {
        DownstreamCode = downstreamCode;
    }

    public String getUpStreamCode() {
        return UpStreamCode;
    }

    public void setUpStreamCode(String upStreamCode) {
        UpStreamCode = upStreamCode;
    }

    public String getiPrintCount() {
        return iPrintCount;
    }

    public void setiPrintCount(String iPrintCount) {
        this.iPrintCount = iPrintCount;
    }

    public String getiVTID() {
        return iVTID;
    }

    public void setiVTID(String iVTID) {
        this.iVTID = iVTID;
    }

    public String getCsysbarcode() {
        return csysbarcode;
    }

    public void setCsysbarcode(String csysbarcode) {
        this.csysbarcode = csysbarcode;
    }

    public String getCborrowouttype() {
        return cborrowouttype;
    }

    public void setCborrowouttype(String cborrowouttype) {
        this.cborrowouttype = cborrowouttype;
    }

    public String getType_id() {
        return type_id;
    }

    public void setType_id(String type_id) {
        this.type_id = type_id;
    }

    public String getRel_obj_type() {
        return rel_obj_type;
    }

    public void setRel_obj_type(String rel_obj_type) {
        this.rel_obj_type = rel_obj_type;
    }

    public String getRel_obj_id() {
        return rel_obj_id;
    }

    public void setRel_obj_id(String rel_obj_id) {
        this.rel_obj_id = rel_obj_id;
    }

    public String getRel_obj_stageid() {
        return rel_obj_stageid;
    }

    public void setRel_obj_stageid(String rel_obj_stageid) {
        this.rel_obj_stageid = rel_obj_stageid;
    }
}
