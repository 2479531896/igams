...
public class XWPFPurchaseUtil{
	...
	private final Logger log = LoggerFactory.getLogger(XWPFPurchaseUtil.class);
	public Map<String,Object> replacePurchase(Map<String,Object> map,IFjcfbService fjcfbService,String FTP_URL,String DOC_OK,AmqpTemplate amqpTempl,String index) {
		...
		try {
			...
			// 临时文件路径
			String tempFilePath = tempPath + BusTypeEnum.IMP_PURCHASE_GENERATE.getCode() + "/" + "UP" + DateUtils.getCustomFomratCurrentDate("yyyy") + "/" + "UP" + DateUtils.getCustomFomratCurrentDate("yyyyMM") + "/"
					+ "UP" + DateUtils.getCustomFomratCurrentDate("yyyyMMdd");
			//正式文件名
			String releaseFilePath = releasePath + BusTypeEnum.IMP_PURCHASE_GENERATE.getCode() + "/" + "UP" + DateUtils.getCustomFomratCurrentDate("yyyy") + "/" + "UP" + DateUtils.getCustomFomratCurrentDate("yyyyMM") + "/"
					+ "UP" + DateUtils.getCustomFomratCurrentDate("yyyyMMdd");
			//文件名
			...
			fjcfbDto.setYwid(map.get("qgid").toString());
			fjcfbDto.setZywid("");
			fjcfbDto.setXh("1");
			fjcfbDto.setYwlx(BusTypeEnum.IMP_PURCHASE_GENERATE.getCode());
			String wjm= map.get("number") +"("+index+")"+".docx";
			fjcfbDto.setWjm(wjm);
			...
			if(isTrue) {
				...
				if(sendFlg) {
					...
					pdfMap.put("ywlx",BusTypeEnum.IMP_PURCHASE_GENERATE.getCode());
...
//					fjcfbDto_t.setYwlx(BusTypeEnum.IMP_PURCHASE.getCode());
//					fjcfbDto_t.setYwid(map.get("qgid").toString());
//					fjcfbService.deleteByYwidAndYwlx(fjcfbDto_t);
		...
		resultMap.put("message", "申购单生成完成");
		resultMap.put("status","success");
		resultMap.put("word_ywlx",BusTypeEnum.IMP_PURCHASE_GENERATE.getCode());
		resultMap.put("pdf_ywlx",BusTypeEnum.IMP_PURCHASE_GENERATE.getCode());
		return resultMap;
	}
	...
	private static void replaceTable(XWPFTable table,Map<String,Object> map) {
		...
		List<Map<String,Object>> listMap=(List<Map<String, Object>>) map.get("qgmx");
//		if(!CollectionUtils.isEmpty(listMap)) {
//			for (int i = 0; i < listMap.size(); i++) {//每条明细都需要生成一个申购单
//				XWPFTableRow row=copy(table, table.getRows().get(1),1+i);
