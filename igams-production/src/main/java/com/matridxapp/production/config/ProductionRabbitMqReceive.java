package com.matridxapp.production.config;
...
@Component
public class ProductionRabbitMqReceive {
	...
	public void DocChangePdf(String str) {
		...
		String fwjlj=p.dCode(map.get("fwjlj"));
		
		mkDirs(fwjlj);
		String fileName= map.get("pdfName");
		//连接服务器
		try{
			//docker无法连接FTP，因为
			...
			if(downflg){
				...
				if(StringUtil.isNotBlank(map.get("fjid"))){
					fjcfbModel.setFjid(map.get("fjid"));
					fjcfbModel.setYwlx(map.get("ywlx"));
				...
				if(t_fjcfbModel != null){
					...
					if(BusTypeEnum.IMP_DOCUMENT.getCode().equals(t_fjcfbModel.getYwlx())){
						...
						t_fjcfbModel.setShrs(map.get("shrList"));
						t_fjcfbModel.setShsjs(map.get("shsjList"));
						t_fjcfbModel.setShyhms(map.get("yhmList"));
						String signflg = map.get("signflg");
						if("0".equals(signflg) || "1".equals(signflg)){
							SyxxDto syxxDto=syxxService.getDtoByWjlb(t_fjcfbModel.getYwlx());
							if(syxxDto!=null) {
								//0:加签名 1:加水印 
								attachHelper.addWatermark(t_fjcfbModel, signflg, map.get("sxrq"),syxxDto);
							}else {
								log.error("水印信息不存在！");
							}
						...
						String replaceflg = map.get("replaceflg");
