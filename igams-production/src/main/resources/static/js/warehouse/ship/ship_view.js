...
var shipments_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#ajaxForm #shipmx_tb_list').bootstrapTable({
            url: $('#ajaxForm #urlPrefix').val()+'/ship/ship/pagedataShipMx',        //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            ...
            uniqueId: "fhmx.xsmxid",                     //每一行的唯一标识，一般为主键列
...
function showXsInfo(value) {
    $.showDialog($("#ajaxForm #urlPrefix").val()+"/storehouse/sale/pagedataViewShipSale?xsid="+value,'详细信息',viewXsConfig);
}
var viewXsConfig = {
    width		: "1600px",
...
var logisticsUpholdView_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#ajaxForm #tb_list').bootstrapTable({
            url:$('#ajaxForm #urlPrefix').val()+'/storehouse/requisition/pagedataGetWlxxByYwid?ywid='+$('#ajaxForm #fhid').val(),         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
function ViewLogisFile(index,zywid) {
    url="/storehouse/requisition/pagedataViewFj?access_token=" + $("#ac_tk").val()+"&zywid="+zywid+"&ywid="+$("#ajaxForm #fhid").val();
...
function wlbmformat(value,row,index){
    ...
    if(row.wlbm==null){
        ...
        html += "<a href='javascript:void(0);' onclick=\"queryBywlid('"+row.wlid+"')\">"+row.wlbm+"</a>";
    }
    return html;
}
