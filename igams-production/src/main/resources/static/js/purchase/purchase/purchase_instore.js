...
var inStore_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#inStoreForm #tb_list').bootstrapTable({
            url: $('#inStoreForm #urlPrefix').val()+'/purchase/purchase/pagedataAdminPurchaseDetailList',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            ...
            onLoadSuccess: function (map) {
                ...
                if(t_map.rows.length>0){
                    ...
                    for(var i = 0; i < t_map.rows.length; i++){
                        ...
                        $("#inStoreForm #qgid").tagsinput('add', {"qgid":value,"djh":text});
                    }
                }
            },
...
function delPurchaseDetail(index,event){
    ...
    $("#inStoreForm #tb_list").bootstrapTable('load',t_map);
}
 ...
 * 监听单据号标签移除事件
 */
var tagsRemoved = $('#inStoreForm #qgid').on('beforeItemRemove', function(event) {
 ...
 * 监听单据号标签点击事件
 */
var tagClick = $("#inStoreForm").on('click','.label-info',function(e){
    event.stopPropagation();
    var url = $('#inStoreForm #urlPrefix').val() + "/purchase/purchase/pagedataChooseListAdminPurchaseInfo?access_token=" + $("#ac_tk").val() + "&qgid=" + e.currentTarget.dataset.logo;
        ...
        success : {
            ...
            callback : function() {
                ...
                if(ids.length > 0){
                    ids = ids.substr(1);
                    // 查询信息并更新到页面
                    $.post($('#inStoreForm #urlPrefix').val() + "/purchase/purchase/pagedataGetPurchaseInfo",{ids:ids,"access_token":$("#ac_tk").val()},function(data){
        ...
        success : {
            ...
            callback : function() {
                ...
                if(sel_row.length==1){
                    ...
                    $.ajax({
                        type:'post',
                        url:$('#inStoreForm #urlPrefix').val() + "/common/habit/commonModUserHabit",
                        cache: false,
 ...
 */
function chooseQg(){
    var url = $('#inStoreForm #urlPrefix').val() + "/purchase/purchase/pagedataChooseAdminPurchaseList?access_token=" + $("#ac_tk").val()+"&zt=80";
