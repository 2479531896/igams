...
var impPurchase_TableInit = function (fjid) {
	...
	oTableInit.Init = function () {
		$('#purchaseImportForm #tb_list').bootstrapTable({
			url: $("#purchaseImportForm #urlPrefix").val()+'/common/file/listImpInfo',		 //请求后台的URL（*）
			method: 'get',					  //请求方式（*）
			toolbar: '#purchaseImportForm #toolbar', //工具按钮用哪个容器
	...
	oTableInit.queryParams = function(params){
		...
		var map = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			fjid: fjid,
			ywlx: $("#purchaseImportForm #ywlx").val(),
		};
...
var showConfig = function(errormsg,infomsg,confirmmsg,warnmsg,successmsg,updList){
	var formname = "#purchaseImportForm";
...
var checkImpCheckStatus = function(intervalTime,fjid){
	...
	$.ajax({
		type : "POST",
		url : $("#purchaseImportForm #urlPrefix").val()+"/common/file/checkImpInfo",
...
var checkImpInsertStatus = function(intervalTime,fjid){
	...
	$.ajax({
		type : "POST",
		url : $("#purchaseImportForm #urlPrefix").val()+"/common/file/checkImpSave",
		data : {"fjid":fjid,"access_token":$("#ac_tk").val()},
		dataType : "json",
		success:function(data){
    ...
    oInits.Init = function () {
    	...
    	$(formname + " #btn-cancel").unbind("click").click(function(){
			...
			$(formname + " #imp_file")
				...
				.hide();
			$('#purchaseImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
			$(formname + " #errordiv").addClass("hidden");
			$(formname + " #infodiv").addClass("hidden");
			$(formname + " #confirmdiv").addClass("hidden");
			$(formname + " #warndiv").addClass("hidden");
    	...
    	$(formname + " #btn-continue").unbind("click").click(function(){
			...
			$(formname + " #btn-continue").addClass("hidden");
			$(formname + " #btn-complete").addClass("hidden");
			var s_fjid = $("#purchaseImportForm #fjid").val();
    		$.ajax({
    			type : "POST",
    			url : $("#purchaseImportForm #urlPrefix").val()+"/common/file/saveImportRecord",
    			data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#purchaseImportForm #ywlx").val()},
    			dataType : "json",
    			success:function(data){
    				if(data.status == "fail")
    				{
    	...
    	$(formname + " #template_dw").unbind("click").click(function(){
    		$("#updatePurchaseForm #access_token").val($("#ac_tk").val());
    		$("#updatePurchaseForm").submit();
    	});
    }
    return oInits;
}
...
function displayUpInfo(fjid){
	$("#purchaseImportForm #fjid").val(fjid);
	//1.初始化Table
	setTimeout("checkImpCheckStatus(2000,'"+ fjid + "')",1000);
}

function qglbinit(){
	var qglblist=$("#purchaseImportForm .qglb");
	if(qglblist!=null && qglblist.length>0){
		for(var i=0;i<qglblist.length;i++){
			var id=qglblist[i].id;
			var csdm=$("#purchaseImportForm #"+id).attr("csdm");
			var qglb=$("#purchaseImportForm #"+id).attr("csid");
			if(csdm=="MATERIAL"){
				$("#purchaseImportForm #"+id).attr("style","text-decoration:none;cursor:pointer;margin-left:10px;background-color:#A9F5A9;color:#fff;");
				$("#purchaseImportForm #qglbdm").val(csdm);
				$("#purchaseImportForm #qglb").val(qglb);
				$("#updatePurchaseForm #wjlx").val("TEMPLATE_IMPORT_QG_WL.xlsx");
			}
		}
	}
}

function chooseQglx(id){
	$("#purchaseImportForm .qglb").attr("style","text-decoration:none;cursor:pointer;margin-left:10px;");
	$("#purchaseImportForm #"+id).attr("style","text-decoration:none;cursor:pointer;margin-left:10px;background-color:#A9F5A9;color:#fff;");
	$("#purchaseImportForm #qglbdm").val($("#purchaseImportForm #"+id).attr("csdm"));
	$("#purchaseImportForm #qglb").val($("#purchaseImportForm #"+id).attr("csid"));
	if($("#purchaseImportForm #"+id).attr("csdm")=="MATERIAL"){
		$("#updatePurchaseForm #wjlx").val("TEMPLATE_IMPORT_QG_WL.xlsx");
	}else if($("#purchaseImportForm #"+id).attr("csdm")=="DEVICE"){
		$("#updatePurchaseForm #wjlx").val("TEMPLATE_IMPORT_QG_SB.xlsx");
	}else if($("#purchaseImportForm #"+id).attr("csdm")=="SERVICE"){
		$("#updatePurchaseForm #wjlx").val("TEMPLATE_IMPORT_QG_FW.xlsx");
	}
}
...
$(function () {
	...
	purchaseImport_params.prefix=$('#purchaseImportForm #urlPrefix').val();
	oFileInput.Init("purchaseImportForm","displayUpInfo",1,1,"imp_file","",purchaseImport_params);
	//2.初始化Button的点击事件
