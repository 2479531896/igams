...
var inStore_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#instoreOtherForm #tb_list').bootstrapTable({
            url: $('#instoreOtherForm #urlPrefix').val()+'/purchase/purchase/pagedataListInstoreOtherList',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            toolbar: '#instoreOtherForm #toolbar',                //工具按钮用哪个容器
            ...
            onLoadSuccess: function (map) {
                ...
                var a=[];
                t_map.rows.push(a);
                $("#instoreOtherForm #tb_list").bootstrapTable('load',t_map);
            },
 ...
 */
function chooseRy(){
    var url=$('#instoreOtherForm #urlPrefix').val() + "/systemmain/task/pagedataCommonListUserPage?access_token=" + $("#ac_tk").val();
    ...
    buttons		: {
        success : {
            ...
            callback : function() {
                ...
                if(sel_row.length==1){
                    $('#instoreOtherForm #rkry').val(sel_row[0].yhid);
                    $('#instoreOtherForm #rkrymc').val(sel_row[0].zsxm);
                    //保存操作习惯
                    $.ajax({
                        type:'post',
                        url:$('#instoreOtherForm #urlPrefix').val() + "/common/habit/commonModUserHabit",
                        cache: false,
...
$(document).ready(function(){
    ...
    init();
    //所有下拉框添加choose样式
    jQuery('#instoreOtherForm .chosen-select').chosen({width: '100%'});
});