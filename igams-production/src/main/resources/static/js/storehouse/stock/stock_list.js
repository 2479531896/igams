var xzbj = $("#stock_formSearch #xzbj").val()
...
var oTableInit = new Object();	    
	...
	oTableInit.Init = function () {
		$('#stock_formSearch #stock_list').bootstrapTable({
			url: $("#stock_formSearch #urlPrefix").val()+'/storehouse/stock/pageGetListStock',         // 请求后台的URL（*）
			method: 'get',                      // 请求方式（*）
...
//				visible: true
				...
				width: '3%',
				align: 'left',
				formatter:stock_formSearch_jyformat,
				visible: true
		    }],
			...
			onDblClickRow: function (row, $element) {
				stockById(row.ckhwid,'view',$("#stock_formSearch #btn_view").attr("tourl"));
			},
		});
	...
	oTableInit.queryParams=function(params){
		var map = {   // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            sortLastName: "ckhwxx.ckhwid", // 防止同名排位用
            sortLastOrder: "asc", // 防止同名排位用
			xzbj: $("#stock_formSearch #xzbj").val(),
			jsid: $("#stock_formSearch #jsid").val()
            // 搜索框使用
...
function stockSearchData(map){
	...
	map["lbs"] = lbs;
    var yxqstart = jQuery('#stock_formSearch #yxqstart').val();
    map["yxqstart"] = yxqstart;
    var yxqend = jQuery('#stock_formSearch #yxqend').val();
    map["yxqend"] = yxqend;
	// 警戒提示
...
//	map["jjtss"] = jjtss;
	...
	map["kczt"] = kczt;
	// 仓库
	var cks = jQuery('#stock_formSearch #ck_id_tj').val();
...
function ArrGoodsSearchData(){
	...
	map["sortLastName"]="ckhwxx.ckhwid";
	map["sortLastOrder"]="desc";
	map["sortName"]="ckhwxx.wlid";
	map["sortOrder"]="desc";
	map["xzbj"]=$("#stock_formSearch #xzbj").val(),
	map["jsid"]=$("#stock_formSearch #jsid").val()
	return stockSearchData(map);
}
...
function stockById(id,action,tourl){
	...
	if(action =='view'){
		...
		var url=tourl+"?ckcsdm=OA"+"&xzbj="+xzbj;;
			...
			callback : function() {
				...
				if(t_map.rows != null && t_map.rows.length > 0){
					for(var i=0;i<t_map.rows.length;i++){
						...
						var sz = {"ckhwid":'',"ckid":'',"wlid":'',"jcsl":'',"bz":'',"yjghrq":''};
    ...
    oInit.Init = function () {
       ...
       /* ---------------------------查看仓库货物-----------------------------------*/
    	btn_view.unbind("click").click(function(){
    		...
    		if(sel_row.length==1){
    			stockById(sel_row[0].ckhwid,"view",btn_view.attr("tourl"));
    		}else{
    			$.error("请选中一行");
    		}
    	});
    	...
    	btn_selectexport.unbind("click").click(function(){
    		...
    		if(sel_row.length>=1){
        		...
        		for (var i = 0; i < sel_row.length; i++) {//循环读取选中行数据
        			ids = ids + ","+ sel_row[i].ckhwid;
        		}
        		ids = ids.substr(1);
        		$.showDialog($('#stock_formSearch #urlPrefix').val()+exportPrepareUrl+"?ywid=STOCK_SELECT&expType=select&ids="+ids
    	...
    	btn_searchexport.unbind("click").click(function(){
    		$.showDialog($('#stock_formSearch #urlPrefix').val()+exportPrepareUrl+"?ywid=STOCK_SEARCH&expType=search&callbackJs=ArrGoodsSearchData"
...
$(function(){
    ...
    laydate.render({
       elem: '#stock_formSearch #yxqend'
      ,theme: '#2381E9'
    });
    //添加日期控件
    laydate.render({
       elem: '#stock_formSearch #yxqstart'
      ,theme: '#2381E9'
    });
	// 所有下拉框添加choose样式
