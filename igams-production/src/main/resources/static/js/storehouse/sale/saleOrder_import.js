...
var impReport_TableInit = function (fjid) {
    ...
    oTableInit.Init = function () {
        $('#orderImportForm #tb_list').bootstrapTable({
            url: $('#orderImportForm #urlPrefix').val()+'/common/file/listImpInfo',		 //请求后台的URL（*）
            method: 'get',					  //请求方式（*）
            toolbar: '#orderImportForm #toolbar', //工具按钮用哪个容器
...
function calculateHsdj(value,row,index){
    var hsdj = (Number(row.hsdj)).toFixed(5);
    return hsdj.toString();
}
//信息显示配置
var showConfig = function(errormsg,infomsg,confirmmsg,warnmsg,successmsg,updList){
    var formname = "#orderImportForm";
...
var checkImpCheckStatus = function(intervalTime,fjid){
    ...
    $.ajax({
        type : "POST",
        url : $('#orderImportForm #urlPrefix').val()+"/common/file/checkImpInfo",
...
var checkImpInsertStatus = function(intervalTime,fjid){
    ...
    $.ajax({
        type : "POST",
        url :$('#orderImportForm #urlPrefix').val()+"/common/file/checkImpSave",
        data : {"fjid":fjid,"access_token":$("#ac_tk").val()},
        dataType : "json",
        success:function(data){
    ...
    oInits.Init = function () {
        ...
        $(formname + " #btn-cancel").unbind("click").click(function(){
            ...
            $(formname + " #imp_file")
                ...
                .hide();
            $('#orderImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
            $(formname + " #errordiv").addClass("hidden");
            $(formname + " #infodiv").addClass("hidden");
            $(formname + " #confirmdiv").addClass("hidden");
            $(formname + " #warndiv").addClass("hidden");
        ...
        $(formname + " #btn-continue").unbind("click").click(function(){
            ...
            $(formname + " #btn-continue").addClass("hidden");
            $(formname + " #btn-complete").addClass("hidden");
            var s_fjid = $("#orderImportForm #fjid").val();
            $.ajax({
                type : "POST",
                url : $('#orderImportForm #urlPrefix').val()+"/common/file/saveImportRecord",
                data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#orderImportForm #ywlx").val()},
                dataType : "json",
                success:function(data){
                    if(data.status == "fail")
                    {
        ...
        //下载模板按钮
        $(formname + " #upload_order").unbind("click").click(function(){
            $("#updateOrderForm #access_token").val($("#ac_tk").val());
...
$(function () {
    ...
    var document_params=[];
    document_params.prefix=$('#orderImportForm #urlPrefix').val();
    oFileInput.Init("orderImportForm","displayUpInfo",1,1,"imp_file","",document_params);
    //2.初始化Button的点击事件
