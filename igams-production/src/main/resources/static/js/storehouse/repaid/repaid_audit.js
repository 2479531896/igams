...
function ghslformat(value,row,index){
    ...
    var html="";
    html ="<div id='ghsldiv_"+index+"' name='ghsldiv' isBeyond='false' style='background:darkcyan;'>";
    html += "<input id='ghsl_"+index+"' type='number' autocomplete='off' value='"+(row.ghsl==null?"0":row.ghsl)+"' name='ghsl_"+index+"'  min='0'  max='"+khsl+"' style='width:55%;border:1px solid #D5D5D5;'";
    html += "onblur=\"checkSum('"+index+"',this,\'ghsl\')\" ";
    html += "></input>";
...
function choosedw(){
    ...
    if(dwdm){
        var url=$('#repaidForm #urlPrefix').val() + "/systemmain/department/pagedataDepartment?access_token=" + $("#ac_tk").val()+"&jgdm="+dwdm;
