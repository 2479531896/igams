...
var logisticsUpholdView_TableInit = function () {
	...
	oTableInit.Init = function () {
		$('#view_ajaxForm #tb_list').bootstrapTable({
			url:$('#view_ajaxForm #urlPrefix').val()+'/storehouse/requisition/pagedataGetWlxxByYwid?ywid='+$('#view_ajaxForm #ywid').val(),         //请求后台的URL（*）
			method: 'get',                      //请求方式（*）
...
function ViewLogisFile(index,zywid) {
	url="/storehouse/requisition/pagedataViewFj?access_token=" + $("#ac_tk").val()+"&zywid="+zywid+"&ywid="+$("#view_ajaxForm #ywid").val();
	$.showDialog($("#view_ajaxForm #urlPrefix").val()+url, '查看附件', viewFileConfig);
}
...
function viewYxht(htid){
	var url=$("#view_ajaxForm #urlPrefix").val()+"/marketingContract/marketingContract/pagedataViewMarketingContract?htid="+htid+"&access_token=" + $("#ac_tk").val();
...
function yl(fjid,wjm){
	...
	if(type.toLowerCase()==".jpg" || type.toLowerCase()==".jpeg" || type.toLowerCase()==".jfif"||type.toLowerCase()==".png"){
		var url=$("#view_ajaxForm #urlPrefix").val()+"/ws/sjxxpripreview?fjid="+fjid
		$.showDialog(url,'图片预览',viewPreViewConfig);
	}else if(type.toLowerCase()==".pdf"){
		var url= $("#view_ajaxForm #urlPrefix").val()+"/common/file/pdfPreview?fjid=" + fjid;
...
function xz(fjid){
	jQuery('<form action="'+$('#view_ajaxForm #urlPrefix').val() + '/common/file/downloadFile" method="POST">' +  // action请求路径及推送方法
