function refreshJydh(){
	var url = $("#inspectionAddForm #urlPrefix").val()+"/inspectionGoods/pendingInspection/pagedataRefreshJydh"
	$.ajax({
		type : "get",
		url: url, 
        ...
        success:function(data){
        	$("#inspectionAddForm #jydh").val(data.jydh);
        	}
        });
}
/**
...
function selectjyfzr(){
	var url=$('#inspectionAddForm #urlPrefix').val() + "/systemmain/task/pagedataCommonListUserPage?access_token=" + $("#ac_tk").val();
	...
	buttons		: {
		success : {
			...
			callback : function() {
				...
				if(sel_row.length==1){
					$('#inspectionAddForm #jyfzr').val(sel_row[0].yhid);
					$('#inspectionAddForm #jyfzrname').val(sel_row[0].zsxm);
		    	}else{
		    		$.error("请选中一行");
		    		return false;
		    	}
...
function displayUpInfo(fjid){
	if(!$("#inspectionAddForm #fjids").val()){
		$("#inspectionAddForm #fjids").val(fjid);
	}else{
		$("#inspectionAddForm #fjids").val($("#inspectionAddForm #fjids").val()+","+fjid);
	}
}
var jychwList_TableInit = function () {
    var oTableInit = new Object();
    //初始化Table
    oTableInit.Init = function () {
        $('#inspectionAddForm #jychwList').bootstrapTable({
            url: $('#inspectionAddForm #urlPrefix').val()+'/inspectionGoods/pendingInspection/pagedataGetGoodsFromJyc',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            toolbar: '#inspectionAddForm #toolbar',                //工具按钮用哪个容器
...
function uploadShoppingFile(index,hwid) {
	if(index){
		var dhjyid = $("#inspectionAddForm #dhjyid").val();
		var fjids = $("#inspectionAddForm #fj_"+ index +" input").val();
		url=$('#inspectionAddForm #urlPrefix').val()+"/inspectionGoods/pendingInspection/pagedataGetUploadFilePage?access_token=" + $("#ac_tk").val()+"&ywid="+dhjyid+"&zywid="+hwid;
		if(fjids){
			url=url + "&fjids="+fjids;
		}
		...
		success : {
			...
			callback : function() {
				...
				if($("#ajaxForm #fjids").val()){
					...
					if(!$("#inspectionAddForm #fj_"+currentUploadIndex+" input").val()){
						$("#inspectionAddForm #fj_"+currentUploadIndex+" input").val($("#ajaxForm #fjids").val());
					}else{
						$("#inspectionAddForm #fj_"+currentUploadIndex+" input").val($("#inspectionAddForm #fj_"+currentUploadIndex+" input").val() + "," + $("#ajaxForm #fjids").val());
					}
					$("#inspectionAddForm #fj_"+currentUploadIndex+" a").text("是");
				}else{
						//
					$("#inspectionAddForm #fj_"+currentUploadIndex+" input").val("");
					$("#inspectionAddForm #fj_"+currentUploadIndex+" a").text("否");
				}
...
function qylformat(value,row,index){
	var qyl = "<input id='qyl"+index+"' type='number' autocomplete='off' name='qyl"+index+"' min='0' max='"+row.maxsl+"' style='width: 100%;' value='"+(row.qyl==null?"0":row.qyl)+"'/>";
	return qyl;
}

//借用数量格式化
function jyslformat(value,row,index){
	var jysl = "<input id='jysl"+index+"' type='number' autocomplete='off' name='jysl"+index+"' min='0' max='"+row.maxsl+"' style='width: 100%;' value='"+(row.jysl==null?"0":row.jysl)+"'/>";
	return jysl;
}
...
function qyrqformat(value,row,index){
	...
	setTimeout(function() {
		laydate.render({
			elem: '#inspectionAddForm #qyrq'+index
		  	,theme: '#2381E9'
		  	,btns: ['clear', 'confirm']
	  		,done: function(value, date, endDate){
	  			$('#inspectionAddForm #qyrq'+index).val(value);
	  		  }
		});
    }, 100);
	return html;
...
function copyTime(index){
	var dqrq=$("#inspectionAddForm #qyrq"+index).val();
	if(dqrq==null || dqrq==""){
		$.confirm("请先选择日期！");
	}else{
		var allTableData = $('#inspectionAddForm #jychwList').bootstrapTable('getData');
		for(var i=0;i<allTableData.length;i++){
			$("#inspectionAddForm #qyrq"+i).val(dqrq);
		}
	}
}
//初验退回数量
...
function removejychw(hwid){
	$('#inspectionAddForm #jychwList').bootstrapTable('remove', {field: 'hwid', values: hwid});
	delhwids.push(hwid);
}
/**
 * 物料编码格式化
...
function queryByWlbm(wlid){
	var url=$("#inspectionAddForm #urlPrefix").val()+"/production/materiel/pagedataViewMater?wlid="+wlid+"&access_token=" + $("#ac_tk").val();
...
$(function () {
	...
	var sflyck = $("#inspectionAddForm #nsflyck").val();
	...
	var tableInit = new jychwList_TableInit();
	tableInit.Init();
});