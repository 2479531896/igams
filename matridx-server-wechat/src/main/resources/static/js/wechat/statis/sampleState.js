var Sample_TableInit=function(){
	...
	oTableInit.Init=function(){
		$("#sample_formSearch  #sample_list").bootstrapTable({
			url: '/wechat/statistics/sampleStatePage',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            toolbar: '#sample_formSearch #toolbar',                //工具按钮用哪个容器
...
function percentage(){
	$.ajax({
		url:'/wechat/statistics/getPercentage',
		type:'post',
		...
		success:function(data){
			if(data.sjxxNum!=null && data.ybztNum!=null){
				...
				str+="%";
				$("#sample_formSearch #num").text($("#ybztmc").text()+"标本："+data.ybztNum+"例");
				$("#sample_formSearch #tage").text(""+"占比："+str);
			}
		}
	})
}
...
function ybzt_back_view(){
	var jsrq=$("#sample_formSearch #jsrq").val();
	$("#daliyStatis").load("/wechat/statistics/getAllDaily?jsrq="+jsrq);
}
 ...
 * @returns
 */
function ybzt_back_week(){
	var jsrqstart = $("#sample_formSearch #jsrqstart").val();
	var jsrqend = $("#sample_formSearch #jsrqend").val();
	$("#weeklyStatis").load("/wechat/statistics/getAllWeekly?jsrqstart="+jsrqstart+"&jsrqend="+jsrqend);
}
