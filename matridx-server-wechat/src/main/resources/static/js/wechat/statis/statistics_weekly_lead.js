...
var loadWeekLeadStatis = function(){
		...
		},
		{
			...
			render:function(data,searchData){
				if(data&&this.chart){
					...
					this.chart.on("click",function(param){
						var load_flg=$("#weeklyStatis #load_flg").val();
						var url="/wechat/statistics/positive_PageList?lxmc="+param.name+"&bgrqstart="+searchData.jsrqstart+"&bgrqend="+searchData.jsrqend+"&bgrqMstart="+searchData.jsrqMstart+"&bgrqMend="+searchData.jsrqMend+"&bgrqYstart="+searchData.jsrqYstart+"&bgrqYend="+searchData.jsrqYend+"&load_flg="+load_flg;
						if(load_flg=="0"){
							dis_toForward(url);
						}else if(load_flg=="1"){
							$("#weeklyStatis").load(url);
	...
	setTimeout(function(){
		...
		//加载数据
		var _url = "/wechat/statistics/getWeekLeadStatis";
		var param ={};
...
function echartsBtnInit(_renderArr){
	...
	$("#weeklyStatis select[name='zfl']").unbind("change").change(function(e){
		...
		$.ajax({ 
			type:'post',  
			url:"/wechat/statistics/getWeekLeadStatisPartner", 
			cache: false,
...
$("#weeklyStatis button[name='hzhb']").unbind("click").click(function(e){
	...
	if(load_flg=="0"){
		jQuery('<form action="/common/view/displayView" method="POST">' +  // action请求路径及推送方法
	            '<input type="text" name="view_url" value="/wechat/statistics/weekLeadStatisPartnerPage?fl='+flid+'&zfl='+hbzfl+'&load_flg='+load_flg+'"/>' + 
	        '</form>')
	    .appendTo('body').submit().remove();
	}else if(load_flg=="1"){
		$("#weeklyStatis").load("/wechat/statistics/weekLeadStatisPartnerPage?fl="+flid+"&zfl="+hbzfl+"&load_flg="+load_flg);
	}
