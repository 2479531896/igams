function xz(fjid,sign){
	jQuery('<form action="/wechat/file/download" method="POST">' +  // action请求路径及推送方法
...
function yl(fjid,wjm){
		...
		if(type.toLowerCase()==".jpg" || type.toLowerCase()==".jpeg" || type.toLowerCase()==".jfif" || type.toLowerCase()==".png"){
			var url="/wechat/wechatpreview/?fjid="+fjid
			$.showDialog(url,'图片预览',JPGMaterConfig);
		}else if(type.toLowerCase()==".pdf"){
			var url= "/wechat/file/pdfView?fjid=" + fjid;
		    $.showDialog(url,'送检报告',JPGMaterConfig);
		}else{
			$.alert("仅支持图片和pdf预览！");
		}
}
...
function saveFeedBack(){
	$("#ajaxForm #saveFeedBackButton").attr('disabled',true);
	if(!$('input:radio[name="sfzq"]:checked').val()){
		$("#ajaxForm #saveFeedBackButton").attr('disabled',false);
		$.toptip('请选择是否正确！', 'warning');
		return false;
	}
	if(!$('#ajaxForm #lcfk').val()){
		$("#ajaxForm #saveFeedBackButton").attr('disabled',false);
		$.toptip('请填写临床反馈！', 'warning');
		return false;
	}
	$.ajax({
    type:"post",
    url:"/wechat/feedback/saveFeedBack",
    dataType:"json",
...
var FileInput = function () {
	...
	oFile.Init = function(ctrlName,callback,impflg,singleFlg,fileName) {
		...
		control.fileinput({
			language: 'zh', //设置语言
			uploadUrl: "/wechat/file/saveImportFile", //上传的地址
...
$(function(){
	...
	oFileInput.Init("ajaxForm","displayUpInfo",2,1,"lcfk_file");
})