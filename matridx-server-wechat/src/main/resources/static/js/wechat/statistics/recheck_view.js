var Recheck_TableInit=function(){
	...
	oTableInit.Init=function(){
		$("#recheck_formSearch_daily  #recheck_list").bootstrapTable({
			url: '/wechat/statistics/recheckListDaily',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
function go_backPage(){
	var lrrys = $("#recheck_formSearch_daily #lrrys").val();
	if(lrrys){
		lrrys = lrrys.substr(1); //删除第一个字符
		lrrys = lrrys.substr(0, lrrys.length-1);
		lrrys = lrrys.split(" ").join("");
	}
	$("#statisticwxyh_daily").load("/wechat/statistics/go_back_dailyByYhid?jsrq="+$("#recheck_formSearch_daily #jsrq").val()+"&yhid="+$("#recheck_formSearch_daily #yhid").val()+"&lrrys="+lrrys);
}
