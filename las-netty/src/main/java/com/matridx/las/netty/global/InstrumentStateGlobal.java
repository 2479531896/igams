...
import com.matridx.las.netty.dao.entities.DesktopMaterialModel;
...
public class InstrumentStateGlobal {
	...
	// 物料全局变量
	private static DesktopMaterialModel desktopMaterialModel = new DesktopMaterialModel();
	//仪器目前接收收到的完成消息
	...
	static {
		instrumentQueues.put(Command.CMH.toString(), new LinkedBlockingQueue<String>());
		instrumentQueues.put(Command.AUTO.toString(), new LinkedBlockingQueue<String>());
	...
	public static void putInstrumentQueuesToRedis(String commd, String deviceId) {
		...
		if (Command.CUBICS.toString().equals(commd)) {
			return;
		} else if (Command.AUTO.toString().equals(commd)) {
			groupStr = "AutoGroup";
	...
	public static void changeInstrumentState(String commd, String deviceId, String state) {
		InstrumentMaterialGlobal.setMaterial(commd, state, deviceId);
	}
	...
	public static DesktopMaterialModel getDesktopMaterialModel() {
		return desktopMaterialModel;
	}
	...
	public void setHaveTakeList(List<String> haveTakeList) {
		this.haveTakeList = haveTakeList;
	}
}
