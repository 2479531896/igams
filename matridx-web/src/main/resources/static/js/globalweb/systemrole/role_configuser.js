...
var role_unselect_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#configUserForm #tb_unlist').bootstrapTable({
            url: '/systemrole/configuser/listUnSelectUser',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
var role_selected_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#configUserForm #tb_list').bootstrapTable({
            url: '/systemrole/configuser/listSelectedUser',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
...
function addOption(){
	...
	if(sel_row.length<1){
		...
		var url = '/systemrole/configuser/toSelected';
		var map = {};
		map["ids"] = ids;
...
function moveOption(){
	...
	if(sel_row.length<1){
		...
		var url = '/systemrole/configuser/toOptional';
		var map = {};
		map["ids"] = ids;
		accessConfigUser(url,map);
	}
