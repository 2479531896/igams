...
var impSale_TableInit = function (fjid) {
	var oTableInit = new Object();
	//初始化Table
	oTableInit.Init = function () {
		$('#saleImportForm #tb_list').bootstrapTable({
			url: '/common/file/listImpInfo',		 //请求后台的URL（*）
			method: 'get',					  //请求方式（*）
			toolbar: '#saleImportForm #toolbar', //工具按钮用哪个容器
		...
		success:function(data){
			if(data.endflg==1){
				if(data.errormsg && data.errormsg!=""){
					...
					//1.初始化Table
					var oTable = new impSale_TableInit();
					oTable.Init();
				}else{
					showConfig(data.infomsg);
					//1.初始化Table
					var oTable = new impSale_TableInit(fjid);
					oTable.Init();
				}
			}else{
				if(data.errormsg && data.errormsg!=""){
...
var saleImp_ButtonInit = function(){
    ...
    oInits.Init = function () {
    	...
    	$(formname + " #btn-cancel").unbind("click").click(function(){
			...
			$(formname + " #sale_file")
				...
				.hide();
			$('#saleImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
			$(formname + " #errordiv").addClass("hidden");
			$(formname + " #infodiv").addClass("hidden");
			$(formname + " #confirmdiv").addClass("hidden");
			$(formname + " #warndiv").addClass("hidden");
    	...
    	$(formname + " #btn-continue").unbind("click").click(function(){
			...
			$(formname + " #btn-continue").addClass("hidden");
			$(formname + " #btn-complete").addClass("hidden");
			var s_fjid = $("#saleImportForm #fjid").val();
    		$.ajax({
    			type : "POST",
    			url : "/common/file/saveImportRecord",
    			data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#saleImportForm #ywlx").val()},
    			dataType : "json",
    			success:function(data){
    				if(data.status == "fail")
    				{
    	...
    	$(formname + " #template_dw").unbind("click").click(function(){
    		$("#updateSaleForm #access_token").val($("#ac_tk").val());
    		$("#updateSaleForm").submit();
    	});
    }
    return oInits;
}
...
$(function () {
	...
	oFileInput.Init("saleImportForm","displayUpInfo",1,1,"sale_file");
	//2.初始化Button的点击事件
    var oimpButtonInit = new saleImp_ButtonInit();
	oimpButtonInit.Init();
});