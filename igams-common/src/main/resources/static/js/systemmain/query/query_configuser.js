...
var role_unselect_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#configUserForm #tb_unlist').bootstrapTable({
            url: $("#configUserForm #urlPrefix").val()+'/systemmain/configuser/pagedataListSelectUser',         //请求后台的URL（*）
            method: 'get',  
    ...
    oTableInit.queryParams = function(params){
    	...
    	var cids = $.trim($("#configUserForm #cids").val());
    	map["cxid"] = $("#configUserForm #cxid").val();
    	map["yhm"] = yhm;
    	map["zsxm"] = zsxm;
    	map["cids"]=cids;
...
var role_selected_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#configUserForm #tb_list').bootstrapTable({
            url: $("#configUserForm #urlPrefix").val()+'/systemmain/configuser/pagedataSelectedUser',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
...
function addOption(){
	...
	if(sel_row.length<1){
		...
		ids = ids.substr(1);
		var cids=$("#configUserForm #cids").val();
		var url = $("#configUserForm #urlPrefix").val()+'/systemmain/configuser/pagedataToSelected';
		var map = {};
		map["ids"] = ids;
...
function moveOption(){
	...
	if(sel_row.length<1){
		...
		ids = ids.substr(1);
		var cids=$("#configUserForm #cids").val();
		var url = $("#configUserForm #urlPrefix").val()+'/systemmain/configuser/pagedataToOptional';
		var map = {};
		map["ids"] = ids;
		map["cids"] = cids;
		accessConfigUser(url,map);
