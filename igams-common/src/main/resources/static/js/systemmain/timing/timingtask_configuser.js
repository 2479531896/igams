...
var role_unselect_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#configUserForm #tb_unlist').bootstrapTable({
            url: $("#configUserForm #urlPrefix").val()+'/systemmain/timing/pagedataListSelectUser',         //请求后台的URL（*）
            method: 'get',  
    ...
    oTableInit.queryParams = function(params){
    	...
    	var rwids = $.trim($("#configUserForm #rwids").val());
    	map["rwid"] = $("#configUserForm #rwid").val();
    	map["yhm"] = yhm;
    	map["zsxm"] = zsxm;
    	map["rwids"]=rwids;
...
var role_selected_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#configUserForm #tb_list').bootstrapTable({
            url: $("#configUserForm #urlPrefix").val()+'/systemmain/timing/pagedataSelectedUser',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
...
function addOption(){
	...
	if(sel_row.length<1){
		...
		ids = ids.substr(1);
		var rwids=$("#configUserForm #rwids").val();
		var url = $("#configUserForm #urlPrefix").val()+'/systemmain/timing/pagedataToSelected';
		var map = {};
		map["ids"] = ids;
        map["lx"] = $("#configUserForm #lx").val();
		map["rwids"] = rwids;
...
function moveOption(){
	...
	if(sel_row.length<1){
		...
		ids = ids.substr(1);
		var rwids=$("#configUserForm #rwids").val();
		var url = $("#configUserForm #urlPrefix").val()+'/systemmain/timing/pagedataToOptional';
		var map = {};
		map["ids"] = ids;
		map["rwids"] = rwids;
		accessConfigUser(url,map);
...
function accessConfigUser(url,map){
	...
	map["rwids"] = $("#configUserForm #rwids").val();
	jQuery.ajaxSetup( {
		async : false
	});
...
$("#configUserForm #lx").change(function(){
    $('#configUserForm #tb_unlist').bootstrapTable('refresh');
});
...
$(function(){
    ...
    oTable2.Init();
    jQuery('#configUserForm .chosen-select').chosen({width: '100%'});
});