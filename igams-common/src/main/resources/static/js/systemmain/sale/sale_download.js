...
function executionDownload() {
	...
	$.ajax({
		type : 'post',
		url : $("#ajaxForm #urlPrefix").val()+"/sale/query/pagedataDownloadQuery",
		cache : false,
		data : {
			"cxid" : cxid,
			"cxtjs" : cxtjs,
			...
			if (data.error != null) {
				...
				if (data.temporary !=null){
					jQuery('<form action="/common/file/downloadFile" method="POST">' +  // action请求路径及推送方法
						'<input type="text" name="wjlj" value="'+data.wjlj+'"/>' +
						'<input type="text" name="wjm" value="'+data.wjm+'"/>' +
						'<input type="text" name="temporary" value="1"/>' +
