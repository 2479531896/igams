 ...
 */
function initPage(){
	var xsfsdm = $("#ajaxForm #xsfsdm").val();
	if(xsfsdm == "Table"){
		$("#ajaxForm #graph").remove();
	}else{
...
var loadPierStatis=function(){
	var myChart = echarts.init(document.getElementById('echarts_sale_statis'));
	var pieoption = {
			tooltip : {
			        trigger: 'item',
		        formatter: "{b} : {c} ({d}%)"
 ...
 * 加载双层圆饼统计图
 */
var loadDoublePierStatis=function(myChart){
	var pieoption = {
			tooltip : {
			    trigger: 'item',
		        formatter: "{b} : {c} ({d}%)"
 ...
 * 加载地图
 */
var loadMapStatis = function(mapdata,leveldata) {
	var myChart = echarts.init(document.getElementById('echarts_sale_statis'));
		...
		var mapoption = {
			...
			tooltip: {
				...
				formatter: function (params) {
					if (params.value){
						return params.name + '\n' +params.value + (params.data.suffix?params.data.suffix:'')
					}else {
						params.name
					}
				},
			...
			visualMap: {
				...
				pieces: leveldata,
				maxOpen: true,
				formatter: function (min_value, max_value,label_value) {
					return min_value + '% ~ ' + max_value + '%(不包含)';
				}
			},
			series:[
				{
...
function executionSelect() {
	...
	var cxtjs = "";
	$("#ajaxForm input[name=cxtj]").each(function() {
		if ($(this).val()) {
			cxtjs = cxtjs + "," + $(this).val();
		} else {
			cxtjs = cxtjs + "," + null;
		}
	});
	cxtjs = cxtjs.substr(1);
	$.ajax({
		type : 'post',
		url : $("#ajaxForm #urlPrefix").val()+"/sale/query/pagedataExecuteQuery",
		cache : false,
		data : {
			"cxid" : cxid,
			"cxtjs" : cxtjs,
		...
		success : function(data) {
			...
			if (data["error"] != null) {
				...
				// 判断显示方式
				if (data.lscxszDto.xsfsdm == "Table") { // 表格
					$("#excTable #bt").empty();
					$("#excTable #nr").empty();
					var zdm = [];
					var bthtml = "";
					...
					for (var i = 0; i < data.headList.length; i++) {
						bthtml = bthtml + "<th title='"+ data.headList[i] + "' style='width:10%;'>" + data.headList[i] + "</th>";
					}
					...
					$("#excTable #bt").append(bthtml);
					$("#excTable #nr").append(nrhtml);
					$("#ajaxForm #excTable").colResizable({
			            liveDrag:true, 
				...
				else if (data.lscxszDto.xsfsdm == "Pie") { // 饼状图
					$("#echarts_sale_statis").css("width", 1143);
					$("#echarts_sale_statis").css("height", 600);
					var myChart = echarts.init(document
							.getElementById('echarts_sale_statis'));
					myChart.resize();
					loadPierStatis();
					...
					for (var i = 0; i < data.resultList.length; i++) {
						seriesData.push({
							value : data.resultList[i].tjsl,
							name : data.resultList[i].tjmc
						});
						legendData.push(data.resultList[i].tjmc);
				...
				else if (data.lscxszDto.xsfsdm == "DoublePie") { // 饼状图
                    $("#echarts_sale_statis").css("width", 1143);
                    $("#echarts_sale_statis").css("height", 600);
                    //显示主题 macarons,infographic,shine,world
                    var myChart = echarts.init(document.getElementById('echarts_sale_statis'),"macarons");
                    myChart.resize();
                    loadDoublePierStatis(myChart);
                    var dataAxis= new Array();
                    var dataseries=new Array();
                    var preFirst = "";
                    var preFirstNum = 0;
                    for (var i = 0; i < data.resultList.length; i++) {
                        dataAxis.push({value:data.resultList[i].secondnum,name:data.resultList[i].secondinfo})
                        if(preFirst != data.resultList[i].firstinfo){
                            if(preFirst != "")
                                dataseries.push({value: preFirstNum,name:preFirst});
                            preFirstNum = parseInt(data.resultList[i].secondnum);
                            preFirst = data.resultList[i].firstinfo;
                        }else{
                            preFirstNum += parseInt(data.resultList[i].secondnum);
                        }
                    }
				...
				else if (data.lscxszDto.xsfsdm == "Bar") { // 柱状图
					$("#echarts_sale_statis").css("width", 1143);
					$("#echarts_sale_statis").css("height", 600);
					var myChart = echarts.init(document.getElementById('echarts_sale_statis'));
					myChart.resize();
					...
					for (var i = 0; i < data.resultList.length; i++) {
						...
						if (Xdata.indexOf(data.resultList[i].tjmc) == -1) {
					...
					if (Xdata.length + count < data.resultList.length) {
						// 组装seriesData
						for (var t = 0; t < data.resultList.length; t++) {
							if (data.resultList[t].tjmc2 != null && seriesData.indexOf(data.resultList[t].tjmc2) == -1) {
								seriesData.push(data.resultList[t].tjmc2);
							}
						}
						// 组装series
						for (var s = 0; s < seriesData.length; s++) {
							...
							for (var k = 0; k < Xdata.length; k++) {
								...
								for (var j = 0; j < data.resultList.length; j++) {
									if (Xdata[k] == data.resultList[j].tjmc && data.resultList[j].tjmc2 == seriesData[s]) {
						...
						var temData = new Array();
						for (var l = 0; l < data.resultList.length; l++) {
							temData.push(data.resultList[l].tjsl);
						}
						Ydata.push({
							name : '数量',
							type : 'bar',
				...
				}else if (data.lscxszDto.xsfsdm == "BarLine") { // 柱状图
                    $("#echarts_sale_statis").css("width", 1143);
                    $("#echarts_sale_statis").css("height", 600);
                    var myChart = echarts.init(document.getElementById('echarts_sale_statis'));
                    myChart.resize();
                    ...
                    var Ydata = new Array();
                    var seriesData = new Array();
                    var Xdata_line = new Array();
                    var Ydataa_line = new Array();
                    var seriesData_line = new Array();
                    // 组装Xdata
                    var count = 0;
                    for (var i = 0; i < data.resultList.length; i++) {
                        if (data.resultList[i].tjmc == null) {
                        ...
                        if (Xdata.indexOf(data.resultList[i].tjmc) == -1) {
                            if(data.resultList[i].tjlx=='BAR'){
                                Xdata.push(data.resultList[i].tjmc);
                            }else if(data.resultList[i].tjlx=='LINE'){
                                 Xdata_line.push(data.resultList[i].tjmc);
                             }
                        }
                    }
                    var BarList=data.resultList.filter(item=> item.tjlx=='BAR')
                    // 判断字段数
                    if (Xdata.length + count < BarList.length) {
                        // 组装seriesData
                        for (var t = 0; t < BarList.length; t++) {
                            if (BarList[t].tjmc2 != null && seriesData.indexOf(BarList[t].tjmc2) == -1) {
                                seriesData.push(BarList[t].tjmc2);
                            }
                        }
                        // 组装series
                        for (var s = 0; s < seriesData.length; s++) {
                            ...
                            for (var k = 0; k < Xdata.length; k++) {
                                ...
                                for (var j = 0; j < BarList.length; j++) {
                                    if (Xdata[k] == BarList[j].tjmc && BarList[j].tjmc2 == seriesData[s]) {
                                        tjsl = BarList[j].tjsl;
                                    }
                                }
                                temData.push(tjsl);
                        ...
                        var temData = new Array();
                        var temData_line = new Array();
                        for (var l = 0; l < data.resultList.filter(item=> item.tjlx=='BAR').length; l++) {
                            temData.push(data.resultList.filter(item=> item.tjlx=='BAR')[l].tjsl);
                        }
                        for (var l = 0; l < data.resultList.filter(item=> item.tjlx=='LINE').length; l++) {
                            temData_line.push(data.resultList.filter(item=> item.tjlx=='LINE')[l].tjsl);
                        }
                        Ydata.push({
                            name : '数量',
                            type : 'bar',
                    ...
                    $("#echarts_sale_statis_bar_line").css("width", 1143);
                    $("#echarts_sale_statis_bar_line").css("height", 600);
                    var myChartBarLine = echarts.init(document.getElementById('echarts_sale_statis_bar_line'));
                    console.log(123)
                    myChartBarLine.setOption({
                         tooltip: {
                            trigger: 'axis'
                          },
				...
				else if (data.lscxszDto.xsfsdm == "Map"){
					$("#echarts_sale_statis").css("width", $("#graph").parent().parent().width()-12);
					$("#echarts_sale_statis").css("height", $("#graph").parent().parent().width() * 0.7);
					//$("#echarts_sale_statis").css("height", $("#graph").parent().parent().height()-148);
					var chinajson = {
					...
					// 处理数据
					for( var j=0;j<data.resultList.length;j++ ){
						if (data.resultList[j].color){
							mapdata.push({
								name:data.resultList[j].tjmc,
								value:data.resultList[j].tjsl,
								itemStyle:{
									normal:{
										areaColor:data.resultList[j].color.replace('＃','#')
									}
								},
								suffix:data.resultList[j].suffix?data.resultList[j].suffix:''
							})
						}else if (data.resultList[j].levelscolor) {
							mapdata.push({
								name:data.resultList[j].tjmc,
								value:data.resultList[j].tjsl,
								suffix:data.resultList[j].suffix?data.resultList[j].suffix:''
							...
							let isExist = false;
							let levels = {color:data.resultList[j].levelscolor.replace('＃','#'),label:data.resultList[j].label_value,min:data.resultList[j].min_value,max:data.resultList[j].max_value}
							if (leveldata.length > 0){
								for(var k=0;k<leveldata.length;k++){
									if (leveldata[k].min == levels.min && leveldata[k].max == levels.max || (leveldata[k].label && levels.label && leveldata[k].label == levels.label )){
										isExist = true
										break;
									}
								}
					...
					//绘制地图
					loadMapStatis(mapdata,leveldata);
				}
			}
			$(".shadow").hide();
			$('.addBox').hide();
			$("#btn_selectexecute").attr("disabled",false);
		}
	});
}

function hsxz(lscxid){
	if (!$("#ajaxForm").valid()) {
		return false;
	}
	var $this = this;
	...
	cxtjs = cxtjs.substr(1);
	console.log("/sale/query/pagedataDownloadHuashanData?access_token="+$("#ac_tk").val()+"&cxid="+cxid+"&cxtjs="+cxtjs)
	window.open("/sale/query/pagedataDownloadHuashanData?access_token="+$("#ac_tk").val()+"&cxid="+cxid+"&cxtjs="+cxtjs)
