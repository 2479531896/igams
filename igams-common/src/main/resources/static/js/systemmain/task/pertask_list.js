...
var task_TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#pertask_formSearch #tb_list').bootstrapTable({
            url:$("#pertask_formSearch #urlprefix").val()+'/systemmain/task/pageGetListPersonalTask',         // 请求后台的URL（*）
            method: 'get',                      // 请求方式（*）
            toolbar: '#pertask_formSearch #toolbar', // 工具按钮用哪个容器
            striped: true,                      // 是否显示行间隔色
            cache: false,                       // 是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   // 是否显示分页（*）
            paginationShowPageGo: true,         //增加跳转页码的显示
            sortable: true,                     // 是否启用排序
            sortName:"gzgl.lrsj",			// 排序字段
            sortOrder: "desc",                   // 排序方式
            ...
            onDblClickRow: function (row, $element) {
                taskDealById(row.gzid, 'view',$("#pertask_formSearch #btn_view").attr("tourl"),row.rwid);
            },
        });
        $("#pertask_formSearch #tb_list").colResizable({
            liveDrag:true,
    ...
    oTableInit.queryParams = function(params){
        ...
        var map = {   // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            sortLastName: "gzgl.xgsj", // 防止同名排位用
            sortLastOrder: "desc" // 防止同名排位用
            // 搜索框使用
            // search:params.search
        };

        var cxtj = $("#pertask_formSearch #cxtj").val();
        // 查询条件
        var cxnr = $.trim(jQuery('#pertask_formSearch #cxnr').val());
        // '0':'任务名称','1':'业务名称',
        if (cxtj == "0") {
            map["rwmc"] = cxnr;
        } else if(cxtj == "1") {
        ...
        // 任务状态
        var zts = jQuery('#pertask_formSearch #zt_id_tj').val();
        map["zts"] = zts;

        // 期望完成开始日期
        var qwwcsjstart = jQuery('#pertask_formSearch #qwwcsjstart').val();
        map["qwwcsjstart"] = qwwcsjstart;

        // 期望完成结束日期
        var qwwcsjend = jQuery('#pertask_formSearch #qwwcsjend').val();
        map["qwwcsjend"] = qwwcsjend;

        // 完成开始日期
        var wcsjstart = jQuery('#pertask_formSearch #wcsjstart').val();
        map["wcsjstart"] = wcsjstart;

        // 完成结束日期
        var wcsjend = jQuery('#pertask_formSearch #wcsjend').val();
        map["wcsjend"] = wcsjend;

        // 录入开始日期
        var lrsjstart = jQuery('#pertask_formSearch #lrsjstart').val();
        map["lrsjstart"] = lrsjstart;

        // 录入结束日期
        var lrsjend = jQuery('#pertask_formSearch #lrsjend').val();
        map["lrsjend"] = lrsjend;
...
function xhFormat(value, row, index) {
    //获取每页显示的数量
    var pageSize=$('#pertask_formSearch #tb_list').bootstrapTable('getOptions').pageSize;
    //获取当前是第几页
    var pageNumber=$('#pertask_formSearch #tb_list').bootstrapTable('getOptions').pageNumber;
...
function taskDealById(id,action,tourl,value){
    ...
    tourl=$("#pertask_formSearch #urlprefix").val()+tourl;
    if(action =='view'){
...
var vm = new Vue({
    ...
    methods: {
        handButton(data){
            var btn_progresssubmit = $("#pertask_formSearch #btn_progresssubmit");
            var btn_taskcare = $("#pertask_formSearch #btn_taskcare");
            var btn_del = $("#pertask_formSearch #btn_del");
            var btn_view = $("#pertask_formSearch #btn_view");
            var btn_mod = $("#pertask_formSearch #btn_mod");

            if(data == "view"){
                var sel_row = $('#pertask_formSearch #tb_list').bootstrapTable('getSelections');//获取选择行数据
                if(sel_row.length==1){
                ...
                $.confirm('您确定要删除所选择的记录吗？',function(result){
                    if(result){
                        ...
                        var url=$("#taskapp1 #urlprefix").val()+btn_del.attr("tourl");
        ...
        searchMore(){
            //添加日期控件
            laydate.render({
                elem: '#pertask_formSearch #qwwcsjstart'
                ,theme: '#2381E9'
            });
            //添加日期控件
            laydate.render({
                elem: '#pertask_formSearch #qwwcsjend'
                ,theme: '#2381E9'
            });
            //添加日期控件
            laydate.render({
                elem: '#pertask_formSearch #wcsjstart'
                ,theme: '#2381E9'
            });
            //添加日期控件
            laydate.render({
                elem: '#pertask_formSearch #wcsjend'
                ,theme: '#2381E9'
            });
            //添加日期控件
            laydate.render({
                elem: '#pertask_formSearch #lrsjstart'
                ,theme: '#2381E9'
            });
            //添加日期控件
            laydate.render({
                elem: '#pertask_formSearch #lrsjend'
                ,theme: '#2381E9'
            });
            var ev=ev||event;
            if(task_turnOff){
                $("#pertask_formSearch #searchMore").slideDown("low");
                task_turnOff=false;
                this.innerHTML="基本筛选";
            }else{
                $("#pertask_formSearch #searchMore").slideUp("low");
                task_turnOff=true;
                this.innerHTML="高级筛选";
            }
...
function searchTaskResult(){
    $('#pertask_formSearch #tb_list').bootstrapTable('refresh');
    //关闭高级搜索条件
    $("#pertask_formSearch #searchMore").slideUp("low");
    task_turnOff=true;
    $("#pertask_formSearch #sl_searchMore").html("高级筛选");
}
...
//	//0.界面初始化
    ...
    // 所有下拉框添加choose样式
    jQuery('#pertask_formSearch .chosen-select').chosen({width: '100%'});

    // 初始绑定显示更多的事件
    $("#pertask_formSearch [name='more']").each(function(){
