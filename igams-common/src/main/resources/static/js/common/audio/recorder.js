(function (window) {  
	...
	var Recorder = function (stream, config) {  
		...
		config.sampleBits = config.sampleBits || 16;	  //采样数位 8, 16  ,缩小文件大小
...
//				return wavd;
		...
		recorder.onaudioprocess = function (e) {
			...
			if(freecont == input.length)
			{
				audioData.emptyTimes ++ ;
				if(audioData.emptyTimes > 20 && audioData.size > 0){
					//停止录音，并上传
					/*recorder.disconnect();
					
					recorder.trans("/common/audio/trans?access_token="+$("#ac_tk").val(), function(responseText,statusText){
						  if(responseText.status != "fail"){
								...
								$audioinfo.val($audioinfo.val()+ responseText.textString);
						  }
					});
