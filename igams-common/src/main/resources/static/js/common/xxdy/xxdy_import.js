...
var impReport_TableInit = function (fjid) {
    ...
    oTableInit.Init = function () {
        $('#xxdyImportForm #tb_list').bootstrapTable({
            url:$("#xxdyImportForm #urlPrefix")+'/common/file/listImpInfo',		 //请求后台的URL（*）
            method: 'get',					  //请求方式（*）
            toolbar: '#xxdyImportForm #toolbar', //工具按钮用哪个容器
...
var checkImpCheckStatus = function(intervalTime,fjid){
    ...
    $.ajax({
        type : "POST",
        url : $("#xxdyImportForm #urlPrefix")+"/common/file/checkImpInfo",
...
var checkImpInsertStatus = function(intervalTime,fjid){
    ...
    $.ajax({
        type : "POST",
        url :$("#xxdyImportForm #urlPrefix")+"/common/file/checkImpSave",
        data : {"fjid":fjid,"access_token":$("#ac_tk").val()},
        dataType : "json",
        success:function(data){
    ...
    oInits.Init = function () {
        ...
        $(formname + " #btn-cancel").unbind("click").click(function(){
            ...
            $(formname + " #imp_file")
                ...
                .hide();
            $('#xxdyImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
            $(formname + " #errordiv").addClass("hidden");
            $(formname + " #infodiv").addClass("hidden");
            $(formname + " #confirmdiv").addClass("hidden");
            $(formname + " #warndiv").addClass("hidden");
        ...
        $(formname + " #btn-continue").unbind("click").click(function(){
            ...
            $(formname + " #btn-continue").addClass("hidden");
            $(formname + " #btn-complete").addClass("hidden");
            var s_fjid = $("#xxdyImportForm #fjid").val();
            $.ajax({
                type : "POST",
                url : $("#xxdyImportForm #urlPrefix")+"/common/file/saveImportRecord",
                data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#xxdyImportForm #ywlx").val()},
                dataType : "json",
                success:function(data){
                    if(data.status == "fail")
                    {
        ...
        //下载模板按钮
        $(formname + " #upload_xxdy").unbind("click").click(function(){
            $("#updateXxdyForm #access_token").val($("#ac_tk").val());
            $("#updateXxdyForm").submit();
        });
    }
    return oInits;
}
...
$(function () {
    ...
    var document_params=[];
    oFileInput.Init("xxdyImportForm","displayUpInfo",1,1,"imp_file","",document_params);
    //2.初始化Button的点击事件
