...
function getViewerConfiguration() {
  return {
    ...
    toolbar: {
      ...
      nextTrain: document.getElementById('nextTrain'),
      onTrain: document.getElementById('onTrain'),
...
var PDFViewerApplication = {
  ...
  bindEvents: function bindEvents() {
    ...
    eventBus.on('nexttrain', webViewernextTrain);
    eventBus.on('ontrain', webVieweronTrain);
  ...
  unbindEvents: function unbindEvents() {
    ...
    eventBus.off('nexttrain', webViewernextTrain);
    eventBus.off('ontrain', webVieweronTrain);
...
function webViewerInitialized() {
  var appConfig = PDFViewerApplication.appConfig;
  var file = void 0;
  var queryString = $("#urlString").val();
  var params = (0, _ui_utils.parseQueryString)(queryString);
  if(!params.file)
	  params.file = $("#pre_fileid").val();
...
function webViewernextTrain() {
  $.ajax({
      type : "post",
      url : $("#urlPrefix").val() +"/ws/task/minidataDingTrainNext",
      data: { "access_token": $("#access_token").val(),"xh":$("#xh").val(),"zlbdm":$("#zlbdm").val(),"ddid":$("#ddid").val(),"gqsjbj": $("#gqsjbj").val(),"listFlag":"1"},
      dataType : "json",
      success : function(datas) {
          if(datas){
              var divNext = document.getElementById("nextTrain");
              var divOn = document.getElementById("onTrain");
              if(datas.flagNext=="0"){
                  $("#xh").val(datas["xh"]);
                  $("#pre_fileid").val(datas["pre_fileid"]);
                  var url = "/common/file/pdfPreviewTrain?fjid="+datas["fjid"]+"&access_token="+$("#access_token").val()
                             +"&syFlag=1&xh="+datas["xh"]+"&ddid="+$("#ddid").val()+"&zlbdm="+$("#zlbdm").val()+"&gqsjbj="+$("#gqsjbj").val();
                  $("#urlString").val(url);
                  document.getElementById("xhSpan").innerHTML = datas["xh"];
                  divNext.className = "btn btn-warning";
                  ...
                  divNext.className = "btn btn-warning disabled";
              }
              if(datas.flag=="0"){
                  divOn.className = "btn btn-warning";
              }else{
                  divOn.className = "btn btn-warning disabled";
              }
          }
      }
  });
...
function webVieweronTrain(){
  $.ajax({
      type : "post",
      url : $("#urlPrefix").val() +"/ws/task/minidataDingTrainNext",
      data: { "access_token": $("#access_token").val(),"xh":$("#xh").val(),"zlbdm":$("#zlbdm").val(),"ddid":$("#ddid").val(),"gqsjbj": $("#gqsjbj").val(),"listFlag":"0"},
      dataType : "json",
      success : function(datas) {
          if(datas){
              var div = document.getElementById("onTrain");
              var divNext = document.getElementById("nextTrain");
              if(datas.flag=="0"){
                  $("#xh").val(datas["xh"]);
                  $("#pre_fileid").val(datas["pre_fileid"]);
...
var defaultOptions = {
  ...
  workerSrc: {
    value: '/js/plugins/pdf/pdf.worker.js',
    kind: OptionKind.WORKER
  }
};
{
...
function attachDOMEventsToEventBus(eventBus) {
  ...
  eventBus.on('pagemode', function (evt) {
    ...
    if(event){
	    ...
	    if(evt.source.pdfViewer && evt.source.pdfViewer.container)
		  ...
		  if(syFlag == '2'){
		    for (var y = 0; y < 5; y++) {
                for (var x = 0; x < 6; x++) {
                  ...
                  c.style.left = (10+x/6*200)+'%';
                  c.style.top = (15+y/5*250)+'%';
                  c.style.transform = "rotate(315deg)";
                  c.style.color = "rgba(0, 0, 0,0.3)";
...
var Toolbar = function () {
  ...
  _createClass(Toolbar, [{
    ...
    value: function _bindListeners() {
      ...
      items.nextTrain.addEventListener('click', function () {
        eventBus.dispatch('nexttrain', { source: self });
      });
      items.onTrain.addEventListener('click', function () {
        eventBus.dispatch('ontrain', { source: self });
      });
...
$(document).ready(function(){
	document.getElementById('panel-body body-content outerContainer').oncontextmenu=new Function("event.returnValue=false");  
	document.getElementById('panel-body body-content outerContainer').onselectstart=new Function("event.returnValue=false");
});
/***/ })
/******/ ]);
//# sourceMappingURL=/js/common/file/viewer.js.map