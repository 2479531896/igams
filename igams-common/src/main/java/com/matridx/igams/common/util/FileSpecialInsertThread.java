package com.matridx.igams.common.util;

import com.matridx.igams.common.service.impl.FileComImport;

public class FileSpecialInsertThread extends Thread{
	
	private final FileComImport fileComImport;
	
	public FileSpecialInsertThread(FileComImport fileComImport){
		this.fileComImport = fileComImport;
		
	}

	@Override
	public void run() {
		try {
			fileComImport.persistentSpecialFileAfterCheck();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}
}
