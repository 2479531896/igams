...
public class CommonXWPFWordUtil {
...
//						}
	...
	private boolean delWaitList(XWPFParagraph paragraph, List<Integer> waitList, String varString) {
		...
		try {
			...
			workbook.setSheetName(0, map.get("rq")+"-"+ map.get("mbmc"));
			...
			String pathname= tempPath + BusTypeEnum.FINANCE_ACCOUNT.getCode() + "/" + map.get("rq") + "-" + map.get("mbmc") + "-" + map.get("ztmc") + "-" + new Date().getTime() + ".xls";
			// 输出Excel文件
...
//			httpResponse.setContentType("application/msexcel");
			...
			redisUtil.hset("EXP_:_"+wjid,"fileName", map.get("rq") + "-" + map.get("mbmc") + "-" + map.get("ztmc") + "-" + new Date().getTime() + ".xls");
	...
	public static void replaceSheetFromTwo(List<Map<String, Object>> listmap, Sheet sheet) {
		try {
			if(listmap!=null&&!listmap.isEmpty()) {
				...
				for (int i = 0; i < listmap.size(); i++) {
					Row row_t = sheet.getRow(i+2);
				for (int j = 0; j < num; j++) {
					Cell cell = row_t.getCell(j);
					if (cell == null || cell.getCellType() != CellType.STRING || cell.getStringCellValue() == null) {
						continue;
					}
					String value = cell.getStringCellValue();
					...
					int endTagPos = value.indexOf("»");
					if (startTagPos > -1 && endTagPos > -1 && endTagPos > startTagPos) {
						cell.setCellValue(replaceValue(value, listmap.get(i), startTagPos, endTagPos));
					}
				} }
			}
		} catch (Exception e) {
