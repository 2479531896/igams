	function initMapPage(){ 
		//定义全局变量，获取ajax中的数据
		var myChart8 = echarts.init(document.getElementById('map'));
		...
		function changeMapdata(data) {
		    ...
		    for(var i=0;i<yblxxxlength;i++){
		    	...
		    	var mddzb = geoCoordMap[yblxxx.yblxlist[i].jcdwdz];
		    	if (geoCoord) {
		            res.push({
		                name: name,
		                ...
		                jcdwdz: yblxxx.yblxlist[i].jcdwdz
		            });
		        }
		    }
		    return res;
		...
		$.ajax({ 
		    ...
		    success:function(data){
		    	...
		    	for(var i=0;i<lxdatalength;i++){
		    		...
		    		movelines["toName"]=lxdata[i].jcdwdz;
		    		...
		    		//处理公司的检测单位，把检测单位单独用其他颜色标识
		    		var result = checkCitys.findIndex(item =>{
		    		    return item.name == lxdata[i].jcdwdz;
		    		});
		    		if(result < 0){
		    			checkCity["name"]=lxdata[i].jcdwdz;
		    			checkCity["value"]=zdzb;
		    			checkCity["value"].push(2);
		    			checkCity["num"]=parseInt(lxdata[i].num);
		    			checkCity["symbolSize"]=10;
		    			checkCity["color"]="#FFD700";
		    			checkCitys.push(checkCity);
		    		}else{
		    			checkCitys[result]["num"] = checkCitys[result].num + parseInt(lxdata[i].num);
		    		}
		    	}
		    	//上一个for循环执行完毕后
		    	for(var k=0;k<csybsjh.length;k++){
		    		...
		    		//如果送检地 和检测单位一样，那么把送检地的合作伙伴信息放入到检测单位信息里，防止看不到信息
		    		var result = checkCitys.findIndex(item =>{
		    		    return item.name == csmc;
		    		});
		    		if(result>=0){
		    			checkCitys[result]["name"]= checkCitys[result]["name"] +": "+ csybsl +"("+ csybsjh[k].db +")";
		    			checkCitys[result]["symbolSize"]=(checkCitys[result]["num"]/sum)*max+2;
		    		}
		    		if(csybsl>1){
		    			cszb.push(2);
		    			citys["value"]=cszb;
