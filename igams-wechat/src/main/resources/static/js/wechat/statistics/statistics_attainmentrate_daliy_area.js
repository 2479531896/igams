 ...
 * @returns
 */
$("#daliyStatis #attainmentrate #rollbackweekly").bind("click",function(){
    $('#daliyStatis').load("/wechat/statistics/pageListLocal_daliy");
});
function loadAttainmentRate(){
    //加载数据
    var _url = "/ws/statistics/getSalesAttainmentRateByArea";
    var param ={
        qymc:$("#daliyStatis #attainmentrate #qymc").val(),
        zblxcsmc:$("#daliyStatis #attainmentrate #zblx").val(),
        yhid:$("#daliyStatis #attainmentrate #yhid").val(),
        kszq:$("#daliyStatis #attainmentrate #searchkszq").val(),
        jszq:$("#daliyStatis #attainmentrate #searchjszq").val(),
        qyid:$("#daliyStatis #attainmentrate #qyid").val(),
        cskz3:$("#daliyStatis #attainmentrate #cskz3").val()
    };
    $.ajax({
        type : "post",
        url : _url,
        ...
        success : function(datas) {
            if(datas){
                ...
                var zblxmc = $("#daliyStatis #zblxmc").val();
                $("#daliyStatis #tatistics_attainmentrate_area").html("");
                ...
                table += "</body>";
                table += "</table></div>";
                $("#daliyStatis #tatistics_attainmentrate_area").append(table);
            }else{
...
function showSalesAttainmentRateDaliyByArea(yhid,zblxmc,qyid,qymc,kszq,jszq){
	...
	if(load_flg=="0"){
		jQuery('<form action="/common/view/displayView" method="POST">' +  // action请求路径及推送方法
	            '<input type="text" name="view_url" value="/ws/statistics/salesAttainmentRateByAreaView?load_flg='+load_flg+'&zblxcsmc='+zblxmc+'&yhid='+yhid+'&qyid='+qyid+'&qymc='+qymc+'&kszq='+kszq+'&jszq='+jszq+'&flag=daliy"/>' + 
	        '</form>')
	    .appendTo('body').submit().remove();
	}else if(load_flg=="1"){
		$("#daliyStatis").load("/ws/statistics/salesAttainmentRateByAreaView?load_flg="+load_flg+"&zblxcsmc="+zblxmc+"&yhid="+yhid+"&qyid="+qyid+"&qymc="+qymc+"&kszq="+kszq+"&jszq="+jszq+"&flag=daliy");
	}
}
function init(){
	//添加日期控件
