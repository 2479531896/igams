var flModels = [];
$("#weekly_Statis_jcdw div[name='echarts_week_lead_jcdwtj']").each(function(){
	var map = {}
...
var loadWeekLeadStatis = function(){
	...
	setTimeout(function(){
		...
		//加载数据
		var _url = "/ws/statistics/getWeekLeadJcdwStatis";
		var param ={};
		param["jsrqstart"]= $("#weekly_Statis_jcdw #jsrqstart").val()
		param["jsrqend"]= $("#weekly_Statis_jcdw #jsrqend").val()
		param["jsrqMstart"]= $("#weekly_Statis_jcdw #jsrqMstart").val()
		param["jsrqMend"]= $("#weekly_Statis_jcdw #jsrqMend").val()
		param["jsrqYstart"]= $("#weekly_Statis_jcdw #jsrqYstart").val()
		param["jsrqYend"]= $("#weekly_Statis_jcdw #jsrqYend").val()
...
function echartsBtnInit(_renderArr){
	...
	map["jsrqstart"]= $("#weekly_Statis_jcdw #jsrqstart").val()
	map["jsrqend"]= $("#weekly_Statis_jcdw #jsrqend").val()

	//所有检测单位测试数的所有统计按钮
	$("#weekly_Statis_jcdw #tj1q").unbind("click").click(function(){
		initEchartsLeadBtnCss("tj1q");
		map["method"]="getAlljcdwByYear";
		clickLeadEcharts(_renderArr,url,map,'jcdwtj');
	})
	//所有检测单位测试数数的月统计按钮
	$("#weekly_Statis_jcdw #tj1m").unbind("click").click(function(){
	...
	//所有检测单位测试数的周统计按钮
	$("#weekly_Statis_jcdw #tj1w").unbind("click").click(function(){
		initEchartsLeadBtnCss("tj1w");
		map["method"]="getAlljcdwByWeek";
		clickLeadEcharts(_renderArr,url,map,'jcdwtj');
	})
	//所有检测单位测试数的日统计按钮
	$("#weekly_Statis_jcdw #tj1d").unbind("click").click(function(){
...
function initEchartsLeadBtnCss(getid,_renderArr){
	var obj = $("#weekly_Statis_jcdw #"+getid)
	var gettitle=obj.attr("title");
	var a=$("#weekly_Statis_jcdw #"+getid).siblings();
	var ah=a.length;
	for(var i=0;i<ah;i++){
		var otherid=a[i].id;
		$("#weekly_Statis_jcdw #"+otherid).removeClass("bechoosed");
		$("#weekly_Statis_jcdw #"+otherid+" .fa").attr("style","");
	}
	//点击统计图按钮，按照全部查询，并显示相应统计图
	//Class="bechoosed"用于处理鼠标移出时导致被选中的按钮样式也被移除的问题
	if(gettitle=="年"){
		$("#weekly_Statis_jcdw #"+getid+" .roundq").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weekly_Statis_jcdw #"+getid).addClass("bechoosed");
	}else if(gettitle=="月"){
		$("#weekly_Statis_jcdw #"+getid+" .roundm").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weekly_Statis_jcdw #"+getid).addClass("bechoosed");
	}else if(gettitle=="周"){
		$("#weekly_Statis_jcdw #"+getid+" .roundw").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weekly_Statis_jcdw #"+getid).addClass("bechoosed");
	}else if(gettitle=="日"){
		$("#weekly_Statis_jcdw #"+getid+" .roundd").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weekly_Statis_jcdw #"+getid).addClass("bechoosed");
	}
}
...
function weekiy_back(){
	...
	$("#weekly_Statis_jcdw").load("/ws/statistics/weekLeadStatisOtherPage?load_flg="+load_flg);
}
