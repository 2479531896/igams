var Sample_TableInit=function(){
	...
	oTableInit.Init=function(){
		$("#sample_jsrq_formSearch  #sample_list").bootstrapTable({
			url: '/ws/sample/sampleStatePage',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            toolbar: '#sample_jsrq_formSearch #toolbar',                //工具按钮用哪个容器
	...
	$.ajax({
		...
		success:function(data){
			if(data.sjxxNum!=null && data.ybztNum!=null){
				...
				str+="%";
				$("#sample_jsrq_formSearch #num").text($("#ybztmc").text()+"标本："+data.ybztNum+"例");
				$("#sample_jsrq_formSearch #tage").text(""+"占比："+str);
			}
		}
	})
}
