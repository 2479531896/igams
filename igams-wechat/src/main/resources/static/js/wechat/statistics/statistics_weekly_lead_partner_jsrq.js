...
var loadWeekLeadStatis = function(){
	...
	var tldb = $("#weekly_Statis_partner_jsrq #tldb").val();
	var tlnum = $("#weekly_Statis_partner_jsrq #tlnum").val();
	...
	var _renderArr = [
  		{
			...
			render:function(data,searchData){
				...
				if ((data||(tldbs!=""&& tldbs.length>0))&&this.chart){
					...
					var tesxdate =  $("#weekly_Statis_partner_jsrq #jsrqstart").val()+"~"+$("#weekly_Statis_partner_jsrq #jsrqend").val();
				if(data&&this.chart){
					var sum = 0;//合计
	...
	setTimeout(function(){
		...
		//加载数据
		var _url = "/ws/statistics/getWeekLeadPartnerStatisByJsrq";
		var param ={};
