var salesModels = [];
//销售达成率
$("#daliyStatis_jsrq div[name='tjsj_salesrate']").each(function () {
	var map = {}
	...
	map["render"] = function (data, searchData) {
		...
		var color = "#00b2ee";
		var xszblist = data;
		$("#daliyStatis_jsrq #xszblist_" + dm).html("");
		var kszq = "";
		var jszq = "";
		if (xszblist != null && xszblist.length > 0) {
			...
			table += "</body>";
			table += "</table></div>";
			$("#daliyStatis_jsrq #xszblist_" + dm).append(table);
		}
		$("#daliyStatis_jsrq #tjsj_t_xszb_"+dm).html("");
		$("#daliyStatis_jsrq #tjsj_t_xszb_"+dm).append(kszq+"~"+jszq);
	};
	...
	var _renderArr = [
		{
			...
			render:function(data,searchData){
				...
				this.chart.on("click",function(param){
					...
					if(load_flg=="0"){
						dis_toForward("/ws/statistics/BbsListViewByJsrq?sfjs="+param.data.sfjs+"&sfsf="+param.data.sfsf+"&jsrq="+searchData.jsrq+"&ybztmc="+param.data.name+"&load_flg="+load_flg);
					}else if(load_flg=="1"){
						$("#daliyStatis_jsrq").load("/ws/statistics/BbsListViewByJsrq?sfjs="+param.data.sfjs+"&sfsf="+param.data.sfsf+"&jsrq="+searchData.jsrq+"&ybztmc="+param.data.name+"&load_flg="+load_flg);
					}
...
//					        data: ['加测申请','1次复测','2次复测']
				...
				this.chart.on("click",function(param){
					var load_flg=$("#load_flg").val();
					if(load_flg=="0"){
						dis_toForward("/ws/statistics/FjsqListViewByJsrq?lx="+param.data.flg+"&lrsj="+searchData.jsrq+"&lxmc="+param.data.name+"&xh="+param.data.xh+"&load_flg="+load_flg);
					}else if(load_flg=="1"){
						$("#daliyStatis_jsrq").load("/ws/statistics/FjsqListViewByJsrq?lx="+param.data.flg+"&lrsj="+searchData.jsrq+"&lxmc="+param.data.name+"&xh="+param.data.xh+"&load_flg="+load_flg+"&jsrq="+$("#daliyStatis_jsrq #jsrq").val());
					}
				});
			}
		},{
			...
			render:function(data,searchData){
				...
				this.chart.on("click",function(param){
					var load_flg=$("#load_flg").val();
					if(load_flg=="0"){
						dis_toForward("/ws/statistics/fqybListViewByJsrq?zt="+param.data.zt+"&jsrq="+searchData.jsrq+"&ybztmc="+param.data.name+"&load_flg="+load_flg);
					}else if(load_flg=="1"){
						$("#daliyStatis_jsrq").load("/ws/statistics/fqybListViewByJsrq?zt="+param.data.zt+"&jsrq="+searchData.jsrq+"&ybztmc="+param.data.name+"&load_flg="+load_flg);
					}
				});
			}
		},{
			...
			render:function(data,searchData){
				...
				this.chart.on("click",function(param){
					var load_flg=$("#load_flg").val();
					if(load_flg=="0"){
						dis_toForward("/ws/statistics/positive_PageList_jsrq?lxmc="+param.data.name+"&bgrq="+searchData.jsrq+"&jyjg="+param.data.jyjg+"&load_flg="+load_flg);
					}else if(load_flg=="1"){
						$("#daliyStatis_jsrq").load("/ws/statistics/positive_PageList_jsrq?lxmc="+param.data.name+"&bgrq="+searchData.jsrq+"&jyjg="+param.data.jyjg+"&load_flg="+load_flg+"&jsrq="+$("#daliyStatis_jsrq #jsrq").val());
					}
			...
			render:function(data,searchData){
				 var table ="<table class='tab'>"
				  ...
				  table+="</table>"
				  if(data.length>0){
					  $("#daliyStatis_jsrq #tjsj_sjxx").append("D："+Dcount+"\xa0\xa0\xa0\xa0"+"R："+Rcount+"\xa0\xa0\xa0\xa0"+"C："+Ccount+"\xa0\xa0\xa0\xa0"+"统计时间：    "+searchData.jsrq)
				  }
			...
			render:function(data,searchData){
				 var table ="<table class='tab'>"
				  ...
				  table+="</table>"
				  if(data.length>0){
					  $("#daliyStatis_jsrq #tjsj_t_sjxx").append("统计时间：    "+searchData.jsrq)
				  }
	...
	setTimeout(function(){
		...
		var _url = "/ws/statistics/getSfsfybxxByJsrq";
		$.ajax({
			type : "post",
			url : _url,
			dataType : "json",
				...
				if(datas){
					...
					for(var i=0; i<_renderArr.length; i++){
						...
						if(_data==null || _data.length==0){
							$("#daliyStatis_jsrq #"+_render.id).parent().hide();
						}
					}
...
$("#jcdw_default").unbind("click").click(function(e){
	...
	if(load_flg=="0"){
		jQuery('<form action="/common/view/displayView" method="POST">' +  // action请求路径及推送方法
	            '<input type="text" name="view_url" value="/ws/statistics/getDailyByJcdwAndJsrq?load_flg='+load_flg+"&jsrq="+$("#jsrq").val()+"&sign="+$("#sign").val()+'"/>' +
	        '</form>')
	    .appendTo('body').submit().remove();
	}else if(load_flg=="1"){
		$("#daliyStatis_jsrq").load("/ws/statistics/getDailyByJcdwAndJsrq?load_flg="+load_flg+"&jsrq="+$("#jsrq").val()+"&sign="+$("#sign").val());
	}
	return true;
});
...
$("#sjhb_default").unbind("click").click(function(e){
	...
	if(load_flg=="0"){
		jQuery('<form action="/common/view/displayView" method="POST">' +  // action请求路径及推送方法
			'<input type="text" name="view_url" value="/ws/statistics/getDailyBySjhbAndJsrq?load_flg='+load_flg+"&jsrq="+$("#jsrq").val()+"&sign="+$("#sign").val()+'"/>' +
			'</form>')
			.appendTo('body').submit().remove();
	}else if(load_flg=="1"){
		$("#daliyStatis_jsrq").load("/ws/statistics/getDailyBySjhbAndJsrq?load_flg="+load_flg+"&jsrq="+$("#jsrq").val()+"&sign="+$("#sign").val());
	}
	return true;
});
...
function qytjlist(){
	var load_flg=$("#load_flg").val();
	$("#daliyStatis_jsrq").load("/ws/statistics/getZoneListByJsrq?load_flg="+load_flg+"&jsrq="+$("#jsrq").val()+"&sign="+$("#sign").val());
}
