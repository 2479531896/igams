var salesModels = [];
//销售达成率
$("#statisticwxyh_daily div[name='tjsj_salesrate']").each(function () {
	var map = {}
	...
	map["render"] = function (data, searchData) {
		...
		var color = "#00b2ee";
		var xszblist = data;
		$("#statisticwxyh_daily #xszblist_" + dm).html("");
		var kszq = "";
		var jszq = "";
		if (xszblist != null && xszblist.length > 0) {
			...
			table += "</body>";
			table += "</table></div>";
			$("#statisticwxyh_daily #xszblist_" + dm).append(table);
		}
		$("#statisticwxyh_daily #tjsj_t_xszb_"+dm).html("");
		$("#statisticwxyh_daily #tjsj_t_xszb_"+dm).append(kszq+"~"+jszq);
	};
...
//					        data: ['加测申请','1次复测','2次复测']
				...
				//点击事件
				this.chart.on("click",function(param){
					$("#statisticwxyh_daily").load("/ws/ststictis/fjsqViewDaily?lx="+param.data.flg+"&lrsj="+searchData.jsrq+"&lxmc="+param.data.name+"&xh="+param.data.xh+"&yhid="+$("#statisticwxyh_daily #yhid").val());
				});
			}
		},{
			id:"fqyb",
			...
			render:function(data,searchData){
				...
				//点击事件
				this.chart.on("click",function(param){
					$("#statisticwxyh_daily").load("/ws/ststictis/fqybViewDaily?zt="+param.data.zt+"&jsrq="+searchData.jsrq+"&ybztmc="+param.data.name+"&yhid="+$("#statisticwxyh_daily #yhid").val());
				});
			}
		},{
			id:"possiblelist",
			...
			render:function(data,searchData){
				...
				//点击事件
				this.chart.on("click",function(param){
					$("#statisticwxyh_daily").load("/ws/ststictis/positiveViewDaily?lxmc="+param.data.name+"&bgrq="+searchData.jsrq+"&jyjg="+param.data.jyjg+"&yhid="+$("#statisticwxyh_daily #yhid").val());
				});
			}
		},{
			id:"sjxxList",
			...
			render:function(data,searchData){
				 var table ="<table class='tab'>"
				  ...
				  table+="</table>"
				  if(data.length>0){
					  $("#statisticwxyh_daily #tjsj_t_sjxx").append("统计时间：    "+searchData.jsrq)
				  }
	...
	setTimeout(function(){
		...
		$.ajax({
			...
			url : _url,
			dataType : "json",
			data:{"sign":$("#statisticwxyh_daily #sign").val(),"jsrq":$("#statisticwxyh_daily #jsrq").val(),"yhid":$("#statisticwxyh_daily #yhid").val()},
				...
				if(datas){
					...
					for(var i=0; i<_renderArr.length; i++){
						...
						if(_data==null || _data.length==0){
							$("#statisticwxyh_daily #"+_render.id).parent().hide();
						}
					}
					...
					if(datas.notSylist==null||datas.notSylist.length==0){
						$("#statisticwxyh_daily #notSylist").parent().hide();
					}
...
var rollback=function(){
	var xsbj=$("#statisticwxyh_daily #xsbj").val();
	if(xsbj=="1"){
		$("#rollback").show();
	}else{
		$("#rollback").hide();
 ...
 * @returns
 */
$("#statisticwxyh_daily #rollback").bind("click",function(){
	var load_flg=$("#load_flg").val();
	$("#statisticwxyh_daily").load("/ws/statistics/getZoneList?load_flg="+load_flg+"&jsrq="+$("#jsrq").val()+"&sign="+$("#sign").val());
})
function showSalesAttainmentRateDaliyByArea(yhid,zblxmc,qyid,qymc,kszq,jszq){
	var jsrq = $("#statisticwxyh_daily #jsrq").val();
	var sign = $("#statisticwxyh_daily #sign").val();
	$("#statisticwxyh_daily").load("/ws/statistics/salesAttainmentRateByAreaView?zblxcsmc="+zblxmc+"&yhid="+yhid+"&qyid="+qyid+"&qymc="+qymc+"&kszq="+kszq
