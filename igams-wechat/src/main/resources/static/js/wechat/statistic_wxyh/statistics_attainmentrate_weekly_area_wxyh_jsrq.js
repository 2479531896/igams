 ...
 * @returns
 */
$("#weeklyStatis_wxyh_jsrq #attainmentrate #rollbackweekly").bind("click",function(){
	var yhid = $("#weeklyStatis_wxyh_jsrq #attainmentrate #parentId").val();
	var jsrqstart = $('#weeklyStatis_wxyh_jsrq #jsrqstart').val();
	var jsrqend = $('#weeklyStatis_wxyh_jsrq #jsrqend').val();
    $("#weeklyStatis_wxyh_jsrq").load("/ws/statictis/backWeeklyByYhidAndJsrq?yhid="+yhid+"&jsrqstart="+jsrqstart+"&jsrqend="+jsrqend);
});
function loadAttainmentRate(){
    //加载数据
    var _url = "/ws/statistics/getSalesAttainmentRateByAreaAndJsrq";
    var param ={
        qymc:$("#weeklyStatis_wxyh_jsrq #attainmentrate #qymc").val(),
        zblxcsmc:$("#weeklyStatis_wxyh_jsrq #attainmentrate #zblx").val(),
        yhid:$("#weeklyStatis_wxyh_jsrq #attainmentrate #yhid").val(),
        kszq:$("#weeklyStatis_wxyh_jsrq #attainmentrate #searchkszq").val(),
        jszq:$("#weeklyStatis_wxyh_jsrq #attainmentrate #searchjszq").val(),
        qyid:$("#weeklyStatis_wxyh_jsrq #attainmentrate #qyid").val()
    };
    $.ajax({
        type : "post",
        url : _url,
        ...
        success : function(datas) {
            if(datas){
                ...
                var zblxmc = $("#weeklyStatis_wxyh_jsrq #zblxmc").val();
                $("#weeklyStatis_wxyh_jsrq #tatistics_attainmentrate_area_jsrq").html("");
                ...
                table += "</body>";
                table += "</table></div>";
                $("#weeklyStatis_wxyh_jsrq #tatistics_attainmentrate_area_jsrq").append(table);
            }else{
...
function showSalesAttainmentRateWeeklyByArea(yhid,zblxmc,qyid,qymc,kszq,jszq){
	var parentId = $("#weeklyStatis_wxyh_jsrq #attainmentrate #parentId").val();
	var jsrqstart = $("#weeklyStatis_wxyh_jsrq #jsrqstart").val();
	var jsrqend = $("#weeklyStatis_wxyh_jsrq #jsrqend").val();
