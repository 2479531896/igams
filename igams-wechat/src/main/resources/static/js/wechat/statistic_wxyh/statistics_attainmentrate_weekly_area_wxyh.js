 ...
 * @returns
 */
$("#weeklyStatis_wxyh #attainmentrate #rollbackweekly").bind("click",function(){
	var yhid = $("#weeklyStatis_wxyh #attainmentrate #parentId").val();
	var jsrqstart = $('#weeklyStatis_wxyh #attainmentrate #jsrqstart').val();
	var jsrqend = $('#weeklyStatis_wxyh #attainmentrate #jsrqend').val();
    $("#weeklyStatis_wxyh").load("/ws/statictis/backWeeklyByYhid?yhid="+yhid+"&jsrqstart="+jsrqstart+"&jsrqend="+jsrqend);
});
function loadAttainmentRate(){
    //加载数据
    var _url = "/ws/statistics/getSalesAttainmentRateByArea";
    var param ={
        qymc:$("#weeklyStatis_wxyh #attainmentrate #qymc").val(),
        zblxcsmc:$("#weeklyStatis_wxyh #attainmentrate #zblx").val(),
        yhid:$("#weeklyStatis_wxyh #attainmentrate #yhid").val(),
        kszq:$("#weeklyStatis_wxyh #attainmentrate #searchkszq").val(),
        jszq:$("#weeklyStatis_wxyh #attainmentrate #searchjszq").val(),
        qyid:$("#weeklyStatis_wxyh #attainmentrate #qyid").val()
    };
    $.ajax({
        type : "post",
        url : _url,
        ...
        success : function(datas) {
            if(datas){
                ...
                var zblxmc = $("#weeklyStatis_wxyh #zblxmc").val();
                $("#weeklyStatis_wxyh #tatistics_attainmentrate_area").html("");
                ...
                table += "</body>";
                table += "</table></div>";
                $("#weeklyStatis_wxyh #tatistics_attainmentrate_area").append(table);
            }else{
...
function showSalesAttainmentRateWeeklyByArea(yhid,zblxmc,qyid,qymc,kszq,jszq){
	var parentId = $("#weeklyStatis_wxyh #attainmentrate #parentId").val();
	...
	$("#weeklyStatis_wxyh").load("/ws/statistics/salesAttainmentRateByAreaView?zblxcsmc="+zblxmc+"&yhid="+yhid+"&qyid="+qyid+"&qymc="+qymc
			+"&kszq="+kszq+"&jszq="+jszq+"&jsrqstart="+jsrqstart+"&jsrqend="+jsrqend+"&parentId="+parentId+"&flag=weekly_wxyh");
}
function init(){
	//添加日期控件
	laydate.render({
