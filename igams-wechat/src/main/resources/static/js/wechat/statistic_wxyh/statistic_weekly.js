var flModels = [];
	...
	//销售达成率
	$("#weeklyStatis_wxyh div[name='salesAttainmentRate']").each(function () {
		var map = {}
		...
		map["render"] = function (data, searchData) {
			...
			var color = "#00b2ee";
			var xszblist = data;
			$("#weeklyStatis_wxyh #echarts_weekly_wxyh_salesAttainmentRate_" + dm).html("");
			if (xszblist != null && xszblist.length > 0) {
				...
				table += "</body>";
				table += "</table></div>";
				$("#weeklyStatis_wxyh #echarts_weekly_wxyh_salesAttainmentRate_" + dm).append(table);
			}
			$("#weeklyStatis_wxyh #tjsj_salesAttainmentRate_"+dm).html("");
			$("#weeklyStatis_wxyh #tjsj_salesAttainmentRate_"+dm).append(searchData.zqs.salesAttainmentRate);
		};
		flModels.push(map);
	});
...
//					        data: ['加测申请','1次复测','2次复测']
				...
				//点击事件
				this.chart.on("click",function(param){
					/*var load_flg=$("#weeklyStatis_wxyh #load_flg").val();
	...
	setTimeout(function(){
			...
			//加载数据
			var _url = "/ws/statictis/getweeklymodel";
			var param ={};
...
function initEchartsLeadBtnCss2(getid,_renderArr){
	...
	//Class="bechoosed"用于处理鼠标移出时导致被选中的按钮样式也被移除的问题
	if(gettitle=="年"){
		$("#weeklyStatis_wxyh #"+getid+" .roundy").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weeklyStatis_wxyh #"+getid).addClass("bechoosed");
	}else if(gettitle=="季度"){
		$("#weeklyStatis_wxyh #"+getid+" .roundq").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weeklyStatis_wxyh #"+getid).addClass("bechoosed");
...
function echartsBtnInit(_renderArr){
	...
	//销售达成率（年）
	$("#weeklyStatis_wxyh a[name='tj9y']").unbind("click").click(function(e){
		initEchartsLeadBtnCss2(e.currentTarget.id);
		map["method"]="getSalesAttainmentRateByYear";
		map["tj"]="year";
		map["qyid"]=$("#weeklyStatis_wxyh #"+e.currentTarget.id).attr("dm");
		clickLeadEcharts(_renderArr,url,map,'salesAttainmentRate_'+$("#weeklyStatis_wxyh #"+e.currentTarget.id).attr("dm"));
	})
	//销售达成率（季度）
	$("#weeklyStatis_wxyh a[name='tj9q']").unbind("click").click(function(e){
		initEchartsLeadBtnCss2(e.currentTarget.id);
	...
	//销售达成率（月）
	$("#weeklyStatis_wxyh a[name='tj9m']").unbind("click").click(function(e){
		initEchartsLeadBtnCss2(e.currentTarget.id);
...
$("#weeklyStatis_wxyh button[name='hzhb']").unbind("click").click(function(e){
	...
	var jsrqstart = $("#weeklyStatis_wxyh #jsrqstart").val();
	var jsrqend = $("#weeklyStatis_wxyh #jsrqend").val();
	var hbzfl = $("#weeklyStatis_wxyh #zfl_" + flid).val();
	var bj="";
	if($("#weeklyStatis_wxyh #xsbj").val()!='1'){
	   bj='1';
	}
	$("#weeklyStatis_wxyh").load("/ws/ststictis/WeeklyPartnerPage?fl="+flid+"&zfl="+hbzfl+"&yhid="+ $("#weeklyStatis_wxyh #yhid").val()+"&sign="+ $("#weeklyStatis_wxyh #t_sign").val()+"&bj="+bj+"&jsrqstart="+jsrqstart+"&jsrqend="+jsrqend);
		return true;
});

$("#weeklyStatis_wxyh #other_sys").unbind("click").click(function(e){
	var yhid=$("#weeklyStatis_wxyh #yhid").val();
	var jsrqstart= $("#weeklyStatis_wxyh #jsrqstart").val();
	var jsrqend= $("#weeklyStatis_wxyh #jsrqend").val();
	var t_sign= $("#weeklyStatis_wxyh #t_sign").val();
...
var weekrollback=function(){
	var xsbj=$("#weeklyStatis_wxyh #xsbj").val();
	if(xsbj=="1"){
		$("#weekrollback").show();
	}else{
		$("#weekrollback").hide();
 ...
 * @returns
 */
$("#weeklyStatis_wxyh #weekrollback").bind("click",function(){
	$("#weeklyStatis_wxyh").load("/ws/statistics/getZoneWeekList?jsrqstart="+$("#jsrqstart").val()+"&jsrqend="+$("#jsrqend").val());
})
function showSalesAttainmentRateWeeklyByArea(yhid,zblxmc,qyid,qymc,kszq,jszq){
	var jsrqstart = $("#weeklyStatis_wxyh #jsrqstart").val();
	var jsrqend = $("#weeklyStatis_wxyh #jsrqend").val();
	var sign = $("#weeklyStatis_wxyh #sign").val();
	$("#weeklyStatis_wxyh").load("/ws/statistics/salesAttainmentRateByAreaView?zblxcsmc="+zblxmc+"&yhid="+yhid+"&qyid="+qyid+"&qymc="+qymc
...
$(function(){
	...
	weekrollback();
	//所有下拉框添加choose样式
	jQuery('#weeklyStatis_wxyh .chosen-select').chosen({width: '100%'});
})