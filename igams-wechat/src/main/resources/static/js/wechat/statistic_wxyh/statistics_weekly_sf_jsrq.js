...
var loadWeekLeadStatis = function(){
	...
	setTimeout(function(){
		...
		//加载数据
		var _url = "/ws/statistics/getWeekWechatProvinceStatisByJsrq";
		var param ={};
		param["jsrqstart"]= $("#weekly_wxyh_sf_jsrq #jsrqstart").val()
		param["jsrqend"]= $("#weekly_wxyh_sf_jsrq #jsrqend").val()
		param["jsrqMstart"]= $("#weekly_wxyh_sf_jsrq #jsrqMstart").val()
		param["jsrqMend"]= $("#weekly_wxyh_sf_jsrq #jsrqMend").val()
		param["jsrqYstart"]= $("#weekly_wxyh_sf_jsrq #jsrqYstart").val()
		param["jsrqYend"]= $("#weekly_wxyh_sf_jsrq #jsrqYend").val()
		param["yhid"]= $("#weekly_wxyh_sf_jsrq #yhid").val();
		param["sf"]=sfid
		$.ajax({
			type : "post",
			url : _url,
	...
	render=_renderArr;
	$("#weekly_wxyh_sf_jsrq .sftj").attr("style","visibility:hidden;");
}
...
function echartsBtnInit(_renderArr){
	...
	map["jsrqstart"]= $("#weekly_wxyh_sf_jsrq #jsrqstart").val()
	map["jsrqend"]= $("#weekly_wxyh_sf_jsrq #jsrqend").val()
	map["sf"]=$("#weekly_wxyh_sf_jsrq #sf").val();
	map["yhid"]= $("#weekly_wxyh_sf_jsrq #yhid").val();
	//标本情况的年统计按钮
	$("#weekly_wxyh_sf_jsrq #tj1q").unbind("click").click(function(){
		initEchartsLeadBtnCss("tj1q");
		map["method"]="getSjxxYearBySyAndSf";
		clickLeadEcharts(_renderArr,url,map,'sfybqk');
	})
	//标本情况的月统计按钮
	$("#weekly_wxyh_sf_jsrq #tj1m").unbind("click").click(function(){
		initEchartsLeadBtnCss("tj1m");
		map["method"]="getSjxxMonBySyAndSf";
		clickLeadEcharts(_renderArr,url,map,'sfybqk');
	})
	//标本情况的周统计按钮
	$("#weekly_wxyh_sf_jsrq #tj1w").unbind("click").click(function(){
		initEchartsLeadBtnCss("tj1w");
		map["method"]="getSjxxWeekBySyAndSf";
		clickLeadEcharts(_renderArr,url,map,'sfybqk');
	})
	//标本情况的日统计按钮
	$("#weekly_wxyh_sf_jsrq #tj1d").unbind("click").click(function(){
	...
	//检测总次数的年统计按钮
	$("#weekly_wxyh_sf_jsrq #tj2q").unbind("click").click(function(){
	...
	//检测总次数的月统计按钮
	$("#weekly_wxyh_sf_jsrq #tj2m").unbind("click").click(function(){
	...
	//检测总次数的周统计按钮
	$("#weekly_wxyh_sf_jsrq #tj2w").unbind("click").click(function(){
	...
	//检测总次数的日统计按钮
	$("#weekly_wxyh_sf_jsrq #tj2d").unbind("click").click(function(){
	...
	//收费标本检测项目次数（年）
	$("#weekly_wxyh_sf_jsrq #tj3q").unbind("click").click(function(){
	...
	//收费标本检测项目次数（月）
	$("#weekly_wxyh_sf_jsrq #tj3m").unbind("click").click(function(){
	...
	//收费标本检测项目次数（周）
	$("#weekly_wxyh_sf_jsrq #tj3w").unbind("click").click(function(){
	...
	//收费标本检测项目次数（日）
	$("#weekly_wxyh_sf_jsrq #tj3d").unbind("click").click(function(){
...
function initEchartsLeadBtnCss(getid,_renderArr){
	var obj = $("#weekly_wxyh_sf_jsrq #"+getid)
	var gettitle=obj.attr("title");
	var a=$("#weekly_wxyh_sf_jsrq #"+getid).siblings();
	var ah=a.length;
	for(var i=0;i<ah;i++){
		var otherid=a[i].id;
		$("#weekly_wxyh_sf_jsrq #"+otherid).removeClass("bechoosed");
		$("#weekly_wxyh_sf_jsrq #"+otherid+" .fa").attr("style","");
	}
	//点击统计图按钮，按照全部查询，并显示相应统计图
	//Class="bechoosed"用于处理鼠标移出时导致被选中的按钮样式也被移除的问题
	if(gettitle=="年"){
		$("#weekly_wxyh_sf_jsrq #"+getid+" .roundq").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weekly_wxyh_sf_jsrq #"+getid).addClass("bechoosed");
	}else if(gettitle=="月"){
		$("#weekly_wxyh_sf_jsrq #"+getid+" .roundm").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weekly_wxyh_sf_jsrq #"+getid).addClass("bechoosed");
	}else if(gettitle=="周"){
		$("#weekly_wxyh_sf_jsrq #"+getid+" .roundw").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weekly_wxyh_sf_jsrq #"+getid).addClass("bechoosed");
	}else if(gettitle=="日"){
		$("#weekly_wxyh_sf_jsrq #"+getid+" .roundd").attr("style","color: #20dbfd;text-shadow: 0 0 25px #00d8ff;");
		$("#weekly_wxyh_sf_jsrq #"+getid).addClass("bechoosed");
	}
}
...
function back(){
	var yhid=$("#weekly_wxyh_sf_jsrq #yhid").val();
	var jsrqstart= $("#weekly_wxyh_sf_jsrq #jsrqstart").val();
	var jsrqend= $("#weekly_wxyh_sf_jsrq #jsrqend").val();
	var t_sign= $("#weekly_wxyh_sf_jsrq #t_sign").val();
...
$(function(){
	...
	loadWeekLeadStatis();
	//所有下拉框添加choose样式
	jQuery('#weekly_wxyh_sf_jsrq .chosen-select').chosen({width: '100%'});
});