var impSjxx_TableInit=function(fjid){
	var oTableInit = new Object();
	oTableInit.Init = function () {
		$('#Sjxx_ImportForm #tb_list').bootstrapTable({
		...
		success:function(data){
			if(data.endflg==1){
				if(data.errormsg && data.errormsg!=""){
					...
					//1.初始化Table
					var oTable = new impSjxx_TableInit();
					oTable.Init();
				}else{
					showConfig(data.infomsg);
					//1.初始化Table
					var oTable = new impSjxx_TableInit(fjid);
					oTable.Init();
				}
			}else{
				if(data.errormsg && data.errormsg!=""){
	...
	$.ajax({
		...
		success:function(data){
			if(data.insertEndflg==1){
				if(data.insertWarn){
					$(formname + " #warnmsg").html(data.insertWarn);
					$(formname + " #infodiv").addClass("hidden");
					$(formname + " #warndiv").removeClass("hidden");
					$(formname + " #btn-complete").removeClass("hidden");
				}else if(data.insertError){
					$(formname + " #errormsg").html(data.insertError);
					$(formname + " #infodiv").addClass("hidden");
					$(formname + " #errordiv").removeClass("hidden");
					$(formname + " #btn-complete").removeClass("hidden");
				}else{
...
var sjxxImp_ButtonInit= function(){
    ...
    oInits.Init = function () {
    	...
    	$(formname + " #btn-cancel").unbind("click").click(function(){
			...
			$(formname + " #imp_file")
				...
				.hide();
			$('#Sjxx_ImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
			$(formname + " #errordiv").addClass("hidden");
			$(formname + " #infodiv").addClass("hidden");
			$(formname + " #confirmdiv").addClass("hidden");
			$(formname + " #warndiv").addClass("hidden");
    	...
    	$(formname + " #btn-continue").unbind("click").click(function(){
			...
			$(formname + " #btn-continue").addClass("hidden");
			$(formname + " #btn-complete").addClass("hidden");
			var s_fjid = $("#Sjxx_ImportForm #fjid").val();
    		$.ajax({
    			type : "POST",
    			url : "/common/file/saveImportRecord",
    			data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),"ywlx": $("#Sjxx_ImportForm #ywlx").val(),"lsbcdz": $("#Sjxx_ImportForm #lsbcdz").val(),"sffs":$('#Sjxx_ImportForm input:radio[name="sffs"]:checked').val()},
    			dataType : "json",
    			success:function(data){
    				if(data.status == "fail")
    				{
    	...
    	//下载模板
    	$(formname + " #upload_btn").unbind("click").click(function(){
    		$("#updateSjxxForm #access_token").val($("#ac_tk").val());
    		$("#updateSjxxForm").submit();
    	});
    }
    return  oInits;
}
...
function displayUpInfo(fjid,lsbcdz){
	$("#Sjxx_ImportForm #fjid").val(fjid);
	$("#Sjxx_ImportForm #lsbcdz").val(lsbcdz);
	//1.初始化Table
...
$(function(){
	...
	oFileInput.Init("Sjxx_ImportForm","displayUpInfo",1,1,"imp_file","");
	//2.初始化Button的点击事件
    var oimpButtonInit = new sjxxImp_ButtonInit();
    oimpButtonInit.Init();
})