...
function loadingWX (){
    //初始化微信信息
    $.ajax({
        url: $("#ajaxForm #menuurl").val()+'/wechat/getJsApiInfo',
        type: 'post',
        data: {
            "url": location.href,
...
function openScan(){
    ...
    wx.scanQRCode({
        ...
        success: function(res) {
            var key=res.resultStr;
            if(key.indexOf("ybbh=")!=-1){
                var indexstart=key.indexOf("ybbh=");
                var str=key.substring(indexstart+5,key.length);
                var indexend=str.indexOf("&");
                if(indexend!=-1){
                    key=str.substring(0,indexend);
                }else{
                    key=str;
                }
            }
