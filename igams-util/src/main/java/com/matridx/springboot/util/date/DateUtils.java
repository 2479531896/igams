...
public class DateUtils {
	...
	private static final HashMap<String,SimpleDateFormat> PRESET_FORMAT = new HashMap<>();
	 ...
	 * @param date date
	 */
	public static String configStringDate(String date){
		String year="";
		String month="";
		String day = "";
		String time = "";
		...
		String now = DateTimeUtil.getFormatDay(new Date());
		if(!date.equals("null")){
			if(date.length()>8){
				...
				return year+tag+month+tag+day+time;
			}
			if(year.equals("0000")){
				year = now.substring(0,4);
			}
			List<String> listMonth;
			listMonth = Arrays.asList(months);
			if(!listMonth.contains(month)){
				month = "01";
			}
			List<String> listDay;
			listDay = Arrays.asList(days);
			if(!listDay.contains(day)&& !day.isEmpty()){
				day = "01";
			}
		}
		return year+tag+month+tag+day+time;
	...
	public static Date configStrDate(String dateStr){
		Date date = null;
		String formatStr = "";
		if(dateStr!=null){
			if(dateStr.length()== 6){
	...
	/**
	 * {@code @Description:校验年份是否可用}
	 * {@code @Author} yujt
	 * {@code @Date} 2017-2-16 下午02:00:38
     */
	public static boolean checkNF(String year){
		boolean result = false;
		try {
			String formatStr = "yyyy-MM-dd";
			SimpleDateFormat sdf = new SimpleDateFormat(formatStr);
			@SuppressWarnings("unused")
			Date date = sdf.parse(year+"-12-12");
			result = true;
		} catch (Exception e) {
			log.error(e.getMessage());
		}
