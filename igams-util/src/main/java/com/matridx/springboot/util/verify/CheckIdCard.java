package com.matridx.springboot.util.verify;
...
public class CheckIdCard{
    ...
    private static final Logger log = LoggerFactory.getLogger(CheckIdCard.class);
    ...
    public static boolean validate(String cardNumber){
        ...
        try{
            Date birthDate = null;
            if (cardNumber != null) {
                birthDate = new SimpleDateFormat(BIRTH_DATE_FORMAT)
            ...
            /*
              ...
              月份和日期相符合
             */
            String birthdayPart = null;
            if (cardNumber != null) {
                birthdayPart = getBirthDayPart(cardNumber);
            }
    ...
    public static long getAge(String cardNumber){
        try {
            if (null != cardNumber){
                cardNumber = cardNumber.trim();
                if (OLD_CARD_NUMBER_LENGTH == cardNumber.length()){
                ...
                //获取身份证上的年月日
                String dates = cardNumber.substring(6, 10) + "-" + cardNumber.substring(10, 12) + "-" + cardNumber.substring(12, 14);
    ...
    public static String getGenger(String cardNumber){//1为男，2为女
        if (null != cardNumber){
            cardNumber = cardNumber.trim();
            if (OLD_CARD_NUMBER_LENGTH == cardNumber.length()){
                cardNumber = contertToNewCardNumber(cardNumber);
            }
            //获取身份证上的倒数第二位数字，奇为男，偶为女
            String genderNumber = cardNumber.substring(cardNumber.length()-2,cardNumber.length()-1);
    ...
    public static void main(String[] args) {
        System.out.println(check("33012519520623311X"));
        System.out.println(getAge("430521199705246140"));
    }
}
