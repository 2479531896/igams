...
public class ReflectionUtils {
	...
	public static  boolean isInterface(Class<?> clazz, Class<?> targetInterface) {
        ...
        for (Class aClass : face) {
            if (aClass.getName().equals(targetInterface.getName())) {
                return true;
            } else {
                Class[] face1 = aClass.getInterfaces();
                for (Class value : face1) {
                    if (value.getName().equals(targetInterface.getName())) {
                        return true;
                    } else if (isInterface(value, targetInterface)) {
                        return true;
                    }
                }
            }
	...
	/**
	 * 直接读取对象属性值（忽略大小写）, 无视private/protected修饰符, 不经过getter函数.
	 */
	public static Object getFieldValueIgnoreCase(final Object obj, final String fieldName) {
		Field field = getAccessibleField(obj, fieldName);
		if(field == null) {
			for (Class<?> superClass = obj.getClass(); superClass != Object.class; superClass = superClass.getSuperclass()) {
		...
		if (field == null) {
			throw new IllegalArgumentException("Could not find field [" + fieldName + "] ignore case on target [" + obj + "]");
		}
