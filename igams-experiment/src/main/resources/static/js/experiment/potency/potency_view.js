$(function(){
	...
	$.ajax({ 
	    type:'post',  
	    url:"/experiment/potency/pagedataView",
	    cache: false,
	    ...
	    success:function(data){
	    	...
	    	for(var i=0;i<data.length;i++){
				...
				$("#xsbs-"+data[i].xh).val(xsbs);
				$("#xsbs-"+data[i].xh).text(xsbs);
...
function printTq(){
    window.open("/experiment/potency/exportPotency?access_token="+$("#ac_tk").val()+"&tqid="+$("#ajaxForm #tqid").val())
}