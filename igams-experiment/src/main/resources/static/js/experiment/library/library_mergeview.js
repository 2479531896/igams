...
$("#wksjxxTitle").click(function(){
    if (flag == 0){
        var wkid = $("#mergeViewAjaxForm #wkid").val();
        var wksjid = $("#mergeViewAjaxForm #wksjid").val();
        $.ajax({
            type:'post',
            ...
            success:function(data){
                ...
                $.each(data.wksjmxlist,function(i,value){
                    ...
                    $("#mergeViewAjaxForm #wksjxx").append(ht);
                });
            }
        });
    }
...
function view(wkid){
    ...
    $.showDialog(url,'文库信息',viewMergeConfig);
}
        ...
        success:function(data){
            ...
            for(var i=0;i<list.length;i++){
                if(list[i].xh%8==0){
                    ...
                    num=8;
                    $("#mergeViewAjaxForm #lie"+lie+"-"+num).text(list[i].jtxx+"--"+list[i].nbbh);
                    $("#mergeViewAjaxForm #lie"+lie+"-"+num).attr("title",list[i].jtxx+"--"+list[i].nbbh);
                    if(list[i].quantity!=null && list[i].quantity!=''){
                        $("#mergeViewAjaxForm #liend"+lie+"-"+num).text(list[i].quantity);
                        $("#mergeViewAjaxForm #liend"+lie+"-"+num).attr("title",list[i].quantity);
                    }
                }else{
