/*var library_TableInit = function (fjid) {
	var oTableInit = new Object();
	//初始化Table
	oTableInit.Init = function () {
		$('#libraryImportForm #tb_list').bootstrapTable({
			url: '/common/file/listImpInfo',		 //请求后台的URL（*）
			method: 'get',					  //请求方式（*）
			toolbar: '#libraryImportForm #toolbar', //工具按钮用哪个容器
			striped: true,					  //是否显示行间隔色
			cache: false,					   //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
			pagination: true,				   //是否显示分页（*）
			sortable: true,					 //是否启用排序
			sortName:"quantity",					//排序字段
			sortOrder: "desc",				   //排序方式
	...
	oTableInit.queryParams = function(params){
		...
		var map = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			fjid: fjid,
			ywlx: $("#libraryImportForm #ywlx").val(),
		};
...
function wkidFormat(){
	var wkid=$("#libraryImportForm #wkid").val();
	return wkid
}

//信息显示配置
var showConfig = function(confirmmsg){
	var formname = "#libraryImportForm";
...
var libraryImp_ButtonInit = function(){
    ...
    oInits.Init = function () {
    	...
    	$(formname + " #btn-cancel").unbind("click").click(function(){
			...
			$(formname + " #goods_file")
				...
				.hide();
			$('#libraryImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
			$(formname + " #errordiv").addClass("hidden");
			$(formname + " #infodiv").addClass("hidden");
			$(formname + " #confirmdiv").addClass("hidden");
			$(formname + " #warndiv").addClass("hidden");
    	...
    	$(formname + " #btn-continue").unbind("click").click(function(){
			...
			$(formname + " #btn-continue").addClass("hidden");
			$(formname + " #btn-complete").addClass("hidden");
			var s_fjid = $("#libraryImportForm #fjid").val();
    		$.ajax({
    			type : "POST",
    			url : "/common/file/saveImportRecord",
    			data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#libraryImportForm #ywlx").val(),impflg:"3"},
    			dataType : "json",
    			success:function(data){
    				if(data.status == "fail")
    				{
    	...
    	$(formname + " #template_dw").unbind("click").click(function(){
    		$("#updateLibraryForm #access_token").val($("#ac_tk").val());
    		$("#updateLibraryForm").submit();
    	});
    }
    return oInits;
}
function displayUpInfo(fjid){
	$("#libraryImportForm #fjid").val(fjid);
	//1.初始化Table
	...
	oFile.Init = function(ctrlName,callback,impflg,singleFlg,fileName,importType) {
		...
		control.fileinput({
			...
			uploadExtraData:function (previewId, index) {
				...
				obj.kzcs1=$("#libraryImportForm #wkid").val();
				return obj;
			},
        ...
        .on('filesuccessremove', function(event, key, jqXHR, data) {
        	if(fileName == 'doc_file' || fileName == 'pro_file' || fileName == 'sign_file'){
...
$(function () {
	...
	oFileInput.Init("libraryImportForm","displayUpInfo",4,1,"library_file");
	//2.初始化Button的点击事件
    var oimpButtonInit = new libraryImp_ButtonInit();
	oimpButtonInit.Init();
});