...
var impReport_TableInit = function (fjid) {
    ...
    oTableInit.Init = function () {
        $('#TrainRecordsImportForm #tb_list').bootstrapTable({
            url: $('#TrainRecordsImportForm #urlPrefix').val() + '/common/file/listImpInfo',		 //请求后台的URL（*）
            method: 'get',					  //请求方式（*）
            toolbar: '#TrainRecordsImportForm #toolbar', //工具按钮用哪个容器
    ...
    oTableInit.queryParams = function(params){
        ...
        var map = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            fjid: fjid,
            ywlx: $("#TrainRecordsImportForm #ywlx").val(),
        };
...
var showConfig = function(errormsg,infomsg,confirmmsg,warnmsg,successmsg,updList){
    var formname = "#TrainRecordsImportForm";
...
var checkImpCheckStatus = function(intervalTime,fjid){
    ...
    $.ajax({
        type : "POST",
        url : $('#TrainRecordsImportForm #urlPrefix').val() + "/common/file/checkImpInfo",
...
var checkImpInsertStatus = function(intervalTime,fjid){
    ...
    $.ajax({
        type : "POST",
        url : $("#TrainRecordsImportForm #urlPrefix").val()+"/common/file/checkImpSave",
        data : {"fjid":fjid,"access_token":$("#ac_tk").val()},
        dataType : "json",
        success:function(data){
    ...
    oInits.Init = function () {
        ...
        $(formname + " #btn-cancel").unbind("click").click(function(){
            ...
            $(formname + " #imp_file")
                ...
                .hide();
            $('#TrainRecordsImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
            $(formname + " #errordiv").addClass("hidden");
            $(formname + " #infodiv").addClass("hidden");
            $(formname + " #confirmdiv").addClass("hidden");
            $(formname + " #warndiv").addClass("hidden");
        ...
        $(formname + " #btn-continue").unbind("click").click(function(){
            ...
            $(formname + " #btn-continue").addClass("hidden");
            $(formname + " #btn-complete").addClass("hidden");
            var s_fjid = $("#TrainRecordsImportForm #fjid").val();
            $.ajax({
                type : "POST",
                url : $("#TrainRecordsImportForm #urlPrefix").val()+"/common/file/saveImportRecord",
                data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#TrainRecordsImportForm #ywlx").val()},
                dataType : "json",
                success:function(data){
                    if(data.status == "fail")
                    {
        ...
        $(formname + " #template_wt").unbind("click").click(function(){
            $("#updateForm #access_token").val($("#ac_tk").val());
            $("#updateForm").submit();
        });
    }
    return oInits;
...
function displayUpInfo(fjid){
    $("#TrainRecordsImportForm #fjid").val(fjid);
    //1.初始化Table
...
$(function () {
    ...
    var document_params=[];
    document_params.prefix=$('#TrainRecordsImportForm #urlPrefix').val();
    oFileInput.Init("TrainRecordsImportForm","displayUpInfo",1,1,"imp_file","",document_params);
    //2.初始化Button的点击事件
