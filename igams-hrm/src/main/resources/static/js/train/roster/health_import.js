...
var impMaterial_TableInit = function (fjid) {
	var oTableInit = new Object();
	//初始化Table
	oTableInit.Init = function () {
		$('#materialImportForm #tb_list').bootstrapTable({
			url: $("#materialImportForm #urlPrefix").val()+'/common/file/listImpInfo',		 //请求后台的URL（*）
			method: 'get',					  //请求方式（*）
			toolbar: '#materialImportForm #toolbar', //工具按钮用哪个容器
	...
	oTableInit.queryParams = function(params){
		...
		var map = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			fjid: fjid,
			ywlx: $("#materialImportForm #ywlx").val(),
		};
...
var showConfig = function(errormsg,infomsg,confirmmsg,warnmsg,successmsg,updList){
	var formname = "#materialImportForm";
...
var checkImpCheckStatus = function(intervalTime,fjid){
	...
	$.ajax({
		type : "POST",
		url : $("#materialImportForm #urlPrefix").val()+"/common/file/checkImpInfo",
		...
		success:function(data){
			if(data.endflg==1){
				if(data.errormsg && data.errormsg!=""){
					...
					//1.初始化Table
					var oTable = new impMaterial_TableInit();
					oTable.Init();
				}else{
					showConfig(data.infomsg);
					//1.初始化Table
					var oTable = new impMaterial_TableInit(fjid);
					oTable.Init();
				}
			}else{
				if(data.errormsg && data.errormsg!=""){
...
var checkImpInsertStatus = function(intervalTime,fjid){
	...
	$.ajax({
		type : "POST",
		url : $("#materialImportForm #urlPrefix").val()+"/common/file/checkImpSave",
		data : {"fjid":fjid,"access_token":$("#ac_tk").val()},
		dataType : "json",
		success:function(data){
    ...
    oInits.Init = function () {
    	...
    	$(formname + " #btn-cancel").unbind("click").click(function(){
			...
			$(formname + " #material_file")
				...
				.hide();
			$('#materialImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
			$(formname + " #errordiv").addClass("hidden");
			$(formname + " #infodiv").addClass("hidden");
			$(formname + " #confirmdiv").addClass("hidden");
			$(formname + " #warndiv").addClass("hidden");
    	...
    	$(formname + " #btn-continue").unbind("click").click(function(){
			...
			$(formname + " #btn-continue").addClass("hidden");
			$(formname + " #btn-complete").addClass("hidden");
			var s_fjid = $("#materialImportForm #fjid").val();
    		$.ajax({
    			type : "POST",
    			url : $("#materialImportForm #urlPrefix").val()+"/common/file/saveImportRecord",
    			data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#materialImportForm #ywlx").val()},
    			dataType : "json",
    			success:function(data){
    				if(data.status == "fail")
    				{
    	...
    	$(formname + " #template_dw").unbind("click").click(function(){
    		$("#updateMaterialForm #access_token").val($("#ac_tk").val());
    		$("#updateMaterialForm").submit();
    	});
    }
    return oInits;
}
...
function displayUpInfo(fjid){
	$("#materialImportForm #fjid").val(fjid);
	//1.初始化Table
...
$(function () {
	...
	var sign_params=[];
	sign_params.prefix=$('#materialImportForm #urlPrefix').val();
	oFileInput.Init("materialImportForm","displayUpInfo",1,1,"material_file",null,sign_params);
	//2.初始化Button的点击事件
    var oimpButtonInit = new materialImp_ButtonInit();
	oimpButtonInit.Init();
});