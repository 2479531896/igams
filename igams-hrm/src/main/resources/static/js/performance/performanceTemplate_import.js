...
var impProduceBom_TableInit = function (fjid) {
    var oTableInit = new Object();
    //初始化Table
    oTableInit.Init = function () {
        $('#performanceTemplateImportForm #tb_list').bootstrapTable({
            url: $('#performanceTemplateImportForm #urlPrefix').val() + '/common/file/listImpInfo',		 //请求后台的URL（*）
            method: 'get',					  //请求方式（*）
            toolbar: '#performanceTemplateImportForm #toolbar', //工具按钮用哪个容器
    ...
    oTableInit.queryParams = function(params){
        ...
        var map = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            fjid: fjid,
            ywlx: $("#performanceTemplateImportForm #ywlx").val(),
        };
...
var showConfig = function(errormsg,infomsg,confirmmsg,warnmsg,successmsg,updList){
    var formname = "#performanceTemplateImportForm";
...
var checkImpCheckStatus = function(intervalTime,fjid){
    ...
    $.ajax({
        type : "POST",
        url : $('#performanceTemplateImportForm #urlPrefix').val() + "/common/file/checkImpInfo",
        ...
        success:function(data){
            if(data.endflg==1){
                if(data.errormsg && data.errormsg!=""){
                    ...
                    //1.初始化Table
                    var oTable = new impProduceBom_TableInit();
                    oTable.Init();
                }else{
                    showConfig(data.infomsg);
                    //1.初始化Table
                    var oTable = new impProduceBom_TableInit(fjid);
                    oTable.Init();
                }
            }else{
                if(data.errormsg && data.errormsg!=""){
...
var checkImpInsertStatus = function(intervalTime,fjid){
    ...
    $.ajax({
        type : "POST",
        url : $("#performanceTemplateImportForm #urlPrefix").val()+"/common/file/checkImpSave",
        data : {"fjid":fjid,"access_token":$("#ac_tk").val()},
        dataType : "json",
        success:function(data){
...
var impProduceBomImp_ButtonInit = function(){
    ...
    oInits.Init = function () {
        ...
        $(formname + " #btn-cancel").unbind("click").click(function(){
            ...
            $(formname + " #imp_file")
                ...
                .hide();
            $('#performanceTemplateImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
            $(formname + " #errordiv").addClass("hidden");
            $(formname + " #infodiv").addClass("hidden");
            $(formname + " #confirmdiv").addClass("hidden");
            $(formname + " #warndiv").addClass("hidden");
        ...
        $(formname + " #btn-continue").unbind("click").click(function(){
            ...
            $(formname + " #btn-continue").addClass("hidden");
            $(formname + " #btn-complete").addClass("hidden");
            var s_fjid = $("#performanceTemplateImportForm #fjid").val();
            $.ajax({
                type : "POST",
                url : $("#performanceTemplateImportForm #urlPrefix").val()+"/common/file/saveImportRecord",
                data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#performanceTemplateImportForm #ywlx").val()},
                dataType : "json",
                success:function(data){
                    if(data.status == "fail")
                    {
        ...
        $(formname + " #template_dw").unbind("click").click(function(){
            $("#updateTemplateForm #access_token").val($("#ac_tk").val());
...
function displayUpInfo(fjid){
    $("#performanceTemplateImportForm #fjid").val(fjid);
    //1.初始化Table
...
$(function () {
    ...
    var document_params=[];
    document_params.prefix=$('#performanceTemplateImportForm #urlPrefix').val();
    oFileInput.Init("performanceTemplateImportForm","displayUpInfo",1,1,"imp_file","",document_params);
    //2.初始化Button的点击事件
    var ProduceBomButtonInit = new impProduceBomImp_ButtonInit();
    ProduceBomButtonInit.Init();
});