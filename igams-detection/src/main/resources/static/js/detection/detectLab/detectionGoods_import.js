var impFzjc_TableInit = function (fjid) {
    ...
    oTableInit.Init = function () {
        $('#detectGoodsImportForm #tb_list').bootstrapTable({
            url: '/common/file/listImpInfo',		 //请求后台的URL（*）
            method: 'get',					  //请求方式（*）
            toolbar: '#detectGoodsImportForm #toolbar', //工具按钮用哪个容器
    ...
    oTableInit.queryParams = function(params){
        ...
        var map = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            fjid: fjid,
            ywlx: $("#detectGoodsImportForm #ywlx").val(),
        };
...
var showConfig = function(errormsg,infomsg,confirmmsg,warnmsg,successmsg,updList){
    var formname = "#detectGoodsImportForm";
    ...
    oInits.Init = function () {
        ...
        $(formname + " #btn-cancel").unbind("click").click(function(){
            ...
            $(formname + " #detection_file")
                ...
                .hide();
            $('#detectGoodsImportForm #update_table').html('<table id="tb_list" class="table table-hover table-bordered"></table>');
            $(formname + " #errordiv").addClass("hidden");
            $(formname + " #infodiv").addClass("hidden");
            $(formname + " #confirmdiv").addClass("hidden");
            $(formname + " #warndiv").addClass("hidden");
        ...
        $(formname + " #btn-continue").unbind("click").click(function(){
            ...
            $(formname + " #btn-continue").addClass("hidden");
            $(formname + " #btn-complete").addClass("hidden");
            var s_fjid = $("#detectGoodsImportForm #fjid").val();
            $.ajax({
                type : "POST",
                url : "/common/file/saveImportRecord",
                data : {"fjid":s_fjid,"access_token":$("#ac_tk").val(),ywlx: $("#detectGoodsImportForm #ywlx").val()},
                dataType : "json",
                success:function(data){
                    if(data.status == "fail")
                    {
...
function displayUpInfo(fjid){
    $("#detectGoodsImportForm #fjid").val(fjid);
    //1.初始化Table
...
$(function () {
    ...
    oFileInput.Init("detectGoodsImportForm","displayUpInfo",1,1,"detection_file");
    //2.初始化Button的点击事件
